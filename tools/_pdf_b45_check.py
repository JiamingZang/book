...
for t in ["图 1-1R 真实数据：同样的追高", "图 1-4R 真实数据：下跌趋势中的尖峰陷阱", "图 1-5R 真实数据：BTC 与 ETH 的相关性不是常数", "图 2-3R 真实数据：尖峰 → 第二波衰竭", "图 2-4R 真实数据：恐慌抛售高潮 → 插针收回", "图 10-4R 真实数据：IV 与 RV 的真实关系", "图 10-5R 真实数据：波动率的分布不是正态",
          ...
          "图 7-1R 真实数据：规则系统的持仓时长", "图 7-2R 真实数据：心态错误的地图", "图 7-3R 真实数据：连亏不是故障，是分布", "图 7-4R 真实数据：报复交易的数学", "图 7-5R 真实数据：一页真实交易日志的统计视图",
          "图 8-4R 真实数据：真实回测的 R 分布",
          "图 10-6R 真实数据：IV Rank 就是你的买卖仪表盘",
          "图 5-2R 真实数据：sweep 失败"]:
