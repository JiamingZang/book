...
3. **SQN −1.42 < 1，直接判死刑**（呼应 8.9）。期望值均值 −0.095R 配 0.44R 的标准差——信号里的优势还没跑赢噪声。按 8.9 的标准，SQN < 1 就该回去改规则（换参数、加过滤器、换入场），而不是调仓位硬扛。
...
*图 8-1 验证闭环：回测→模拟盘→小资金实盘→复盘，不达标回到回测*
...
![图 8-2 复盘体系流水线：日复盘（15 分钟）→ 周复盘（1 小时）→ 月复盘（半天）→ 决策反馈（继续/优化/暂停）回到日复盘——复盘是持续验证的引擎](images/fig_p8_review_flow.png)

*图 8-2 复盘体系流水线：日复盘（15 分钟）→ 周复盘（1 小时）→ 月复盘（半天）→ 决策反馈（继续/优化/暂停）回到日复盘——复盘是持续验证的引擎*

图 8-2 把三层节奏画成一条流水线，**每层各干各的，不要越权**：
...
![图 8-3 R 倍数分布：大多数交易亏 -1R，少数大 R 撑起整体期望（SQN 的输入）](images/fig_p8_r_dist.png)

*图 8-3 R 倍数分布：大多数交易亏 -1R，少数大 R 撑起整体期望（SQN 的输入）*
...
![图 8-4 绩效归因：左=资金曲线 100 笔累计约 +10%（一次连亏回撤后 2-3 周恢复）；右=setup 归因——趋势回调贡献 76% 利润，负贡献的突破该砍](images/fig_p8_attribution.png)

*图 8-4 绩效归因：左=资金曲线 100 笔累计约 +10%（一次连亏回撤后 2-3 周恢复）；右=setup 归因——趋势回调贡献 76% 利润，负贡献的突破该砍*

左右对着读：

- **左图回答"整体稳不稳"**——100 笔单笔风险 0.5%，第 36-42 笔六连亏回撤 3.5%，但 2-3 周就收复失地——这正是 6.3 说的"连亏是必然，只要不放大"；
...
![图 8-5 拍卖四环节：趋势→停止→震荡→转换，平衡与失衡循环往复](images/fig_p8_auction_cycle.png)

*图 8-5 拍卖四环节：趋势→停止→震荡→转换，平衡与失衡循环往复*
...
![图 8-6 足迹图：把每根 K 线的量按价格档拆开（左买右卖），失衡档高亮；三种读法——失衡/变盘/延续](images/fig_p8_footprint.png)

*图 8-6 足迹图：把每根 K 线的量按价格档拆开（左买右卖），失衡档高亮；三种读法——失衡/变盘/延续*

（左）一根大阳 K 线的逐档拆解：每一档标出"主动买量/主动卖量"——100.4 档买 60 卖 10 是失衡（橙色高亮）：单边吃单、价格快速通过，之后倾向回填；101.0 上影档卖量 12 压顶是上攻受阻信号。（右）三种读法：失衡 → 快速通过区；变盘 → 努力无结果（反转警告）；延续 → 趋势健康。**与图 8-7 互证**：足迹失衡 ≈ 单根 delta 大幅转正，变盘 ≈ delta 衰竭——同一个过程的两套读数。
...
![图 8-7 订单流三件套：DOM 静态地图 / 逐笔动态新闻 / Delta 记分牌（合成示意）](images/fig_p8_orderflow.png)

*图 8-7 订单流三件套：DOM 静态地图 / 逐笔动态新闻 / Delta 记分牌（合成示意）*
...
1. **上真钱门槛**：100 笔模拟 + 正期望值 + 执行率 > 80%（8.7），三个条件缺一不上真钱；
