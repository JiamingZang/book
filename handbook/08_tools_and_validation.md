...
**上面是流程，下面用真实数据把"验证为什么必须严格"实证一遍**——把图 8-1R 的策略族（EMA 均线趋势跟踪，BTC 1H，2026-07-02 ~ 08-10 同一段行情）换成参数扫描：快线 {5,10,15,20,25,30,40,50} × 慢线 {50,75,100,150,200}，40 组参数逐组回测：
...
![图 8-4R 真实数据：真实回测的 R 分布——止损纪律的指纹 + 赢在尾部（上=82 笔 R 直方图，红=亏绿=赢：亏单全被止损锁死在 −1R 内（−0.91~−1.00，滑点+止损微超）、赢单右尾最长 +2.85R，均值 −0.10R、赢单均值 +0.41R、亏单均值 −0.44R；下=按 R 从大到小逐笔柱+累计 R 线：前 5 大赢单 +7.79R = 赢单总利润的 57%、去掉它们 → −15.6R、82 笔终点 −7.83R 平均 −0.10R/笔 验证不合格）（数据源：同图 8-1R 的 82 笔真实记录）](images/fig_real_ch8_rdist.png)

*图 8-4R 真实数据：真实回测的 R 分布——亏单被止损锁死在 −1R 内（纪律的指纹），赢靠少数大 R 撑起（数据源：同图 8-1R 的 82 笔真实记录）*

**这张图读出三件事**（图 8-8 的真实数据版）：

1. **止损纪律的指纹：49 笔亏损全部锁死在 −1R 内**。直方图左半边像一堵墙——没有一根“扛单巨亏”，最亏也就 −1.00R（−0.91~−1.00 是滑点+止损微超）。这就是“入场即挂止损”的物理证据：系统级风险是可控的，亏损分布是“闭眼可预期”的（呼应 6.4 止损与图 8-1R 的 −1R 锁死）。合成图 8-8 画的“大多数亏 -1R”不是夸张，是纪律系统的真实形态。
...
![图 8-5R 真实数据：拍卖理论的真实循环——趋势 → 停止 → 平衡 → 失衡 → 接受（上=K 线五段色带：早盘下跌后 09:40-13:40 趋势推升 58771→59374（失衡）；13:40-16:40 冲高后大量反向交易、回落至 58500（停止 A）；16:40-20:50 横盘 58350-58990 近 4 小时、下沿 3 次测试（平衡=价值区，量能温和 30-80）；20:50-21:12 巨量 V=635（常态 16 倍）把价格推离下沿（失衡）；21:12-21:40 回踩 58602 守住、收盘不回下沿（接受）；21:40 后放量 V=383 突破上沿 58990 → 59410 新高（转换，趋势展开）；下=成交量：平衡区量能温和、失衡与转换才放量）（数据源：Binance BTCUSDT 5m）](images/fig_real_ch8_auction.png)
...
- **（③）接受 vs 拒绝是转换的分水岭**：21:15-21:35 价格回踩 58602，收盘守住不回下沿 = 接受（参与者接受新价位，新价值区成立）；对比图 4-2R 的区间假突破——插破上沿又收回来 = 拒绝。同一个“离开平衡区”的动作，接受 → 趋势展开（21:45 放量突破上沿 58990 → 59410 新高），拒绝 → 反向破位。**接受/拒绝的判定 = 突破真假判断的理论版**（3.2 收盘确认 + 放量）。
...
**成交量极端柱（VX）与动态开盘区间（Ali）**：足迹图家族里还有一种"只看成交量极值"的简化法——**VX 柱 = 成交量极端柱**（如 3 分钟图上成交量超过 2.5 万的柱体；阈值按品种校准，ES 可用 5 分钟图 4 万+/3 万+ 两级）——**VX 柱的极值点（高点/低点/中点）具有磁吸效应，构成支撑/阻力位**；当日成交量最大的巨型趋势棒通常决定后续价格波动的范围。两级用法：**第一级 VX 柱定主要目标位，第二级 VX 柱定日内支撑/阻力位**（柱的高/低/中点三档）；**控制柱（CB）** = 相对前后柱体成交量显著但未达 VX 值的柱体，常常影响并限制后续多根柱体的走势（在交易区间 TR 中尤为重要）。**动态开盘区间**：用 3/5 分钟图前几根 K 线的成交量确定开盘区——成交量超过阈值（如 25K）的 K 线范围即开盘区间，**基于开盘区间的突破通常能达到测量移动目标**（呼应 4.26 ORB：开盘区间突破量度）。**失败处理**：若市场在触及 VX 目标前反转，通常测试反向目标、且常触及 2 倍反向目标（趋势交易日更常见）——VX 方向未达 = 反向信号（呼应 8.13 的 TX-D 失效后反向）。
...
| CCTK-EB（提前入场） | CCTK 信号作为“入场柱”（EB）而非首个信号柱时，用限价单提前入场 | 在波动小的市场中及时了结，或在新趋势未完全确立时用紧密止损布局 |
| IK Tick Hit（首千次命中） | 跳动读数首次冲到千次级别的信号（低动量 700 / 高动量 1200） | 首次触发先顺势；仅在 LL/HH + Tick 读数低时才反转 |
| TX 1200（千点刻度） | 跳动读数 1200 的极值水平 | 突破初期=中段信号（回调后顺势）；趋势末期=最后一冲（反向短线） |
...
**TX-D 失效时的应对（Ali）**：约 2/3 的情况失效后市场先横盘震荡数根 K 线（允许交易管理）；约 1/3 的情况剧烈反转（二段趋势 2LT 信号中更常见，此时无法实施管理）。**TX-D 失效通常由真空效应导致**（趋势区间测试阻力位附近时）；**带过滤器的 TX-D 信号失败后，反而是反向交易的更可靠信号**。

**首个千次点击命中（IK Tick Hit，Ali）**：当跳动读数首次冲到"千次"级别（Tick 计数达到 700/1200 量级）时，市场通常已经运行了一段距离——**IK Tick Hit 标记"卖出反弹（BR）"或"买入回调（PB）"机会：寻找 Tick 读数较低的二次测试（2Test），价格行为确认成立后做反转交易**。规则：**首次 IK 刻度触发突破后先跟随趋势（BO）**——**仅在出现更低低点（BR 侧）/更高高点（BL 侧）且 Tick 读数较低时，才考虑反转**。Tick 在这里是过滤器：**用于筛选"弱势 PA 形态"或"看似良好时机但过早的形态"**——Tick 读数高说明失衡还在持续（别急着反手），Tick 读数低说明失衡开始耗尽（反转条件成熟）。级别选择：**低动量市场用 700 Tick 级别，高动量市场用 1200 Tick 级别**——规则相同，只是放大的尺度不同（呼应 4.6 微通道：趋势越强，可容忍的反向脉冲越大）。

**TX 1200：千点刻度的"最后一冲还是中段"（Ali）**：跳动读数冲到 1200 时，有时是 BL 阶段的**最后一冲**，有时只是**中段**——判断靠价格行为背景，不靠读数本身：
...
- **趋势末期/最终冲刺**：TX 1200 出现在趋势末端且 K 线形态符合入场条件时，做反向短线——这是"最后一冲"的信号，不是趋势延续（呼应 4.10 突破生命周期：末段突破=高潮）。
- **基底测试**：市场通常先测试最终腿的 TK 基底（Tick 基底），然后要么反转、要么横盘、偶尔向下突破——**别在测试基底前抢方向**。
- **注意**：形态出现在交易日最初或最后几根 K 线时可靠性降低（盘前活动/收盘调仓影响 Tick 行为）；新闻事件日（如 FOMC）突破阶段可能连续出现多个 TX 1200 形态——规则相同，但按"多个形态"处理（不合并为一个信号）。

**HTK/LTK：当日最高/最低成交价的磁吸作用（Ali 卡片）**：HTK（Highest Traded Price）与 LTK（Lowest Traded Price）是当日盘中**实际成交过**的最高/最低价位——注意与当日最高点/最低点（HOD/LOD）区分：HTK 很少恰好成为当日最高点，LTK 很少恰好成为当日最低点（极值常常以"无人接单"的瞬间刺破形式闪过）。三个交易应用：① **至少会再测试**——市场通常会回头测试出现极端 Tick 的价位水平，但多数情况下会**突破**该价位继续走；② **TRD 中的磁吸**——趋势性交易区间日里，趋势推动当日极端价格与 Ticks 同步创新高（新交易区间会暂时逆转极端 Tick 的走势，但随后会尝试再次测试该点位），因此 HTK/LTK 是 TRD 日里的天然磁吸位，可以直接当目标位用；③ **与 TX 的分工**——TX 是 Tick 图上"画出来的"极值水平，HTK/LTK 是"实际成交出来的"极值价位，两者互相印证时信号更强（呼应 8.11 足迹图的逐价成交：成交分布最密的价位才是市场认可的价位）。
...
8. **日志两阶段**：完整模板 14 列——前 9 列在入场前填（计划：日期/品种/方向/背景/信号/入场价/止损价/目标价/仓位），后 5 列在平仓后填（结果与反思：结果R/是否按计划/心理状态/截图链接/复盘备注）（8.3）；先计划后结果，入场判断被固定下来，复盘才能客观对照——**执行率 = “是否按计划 = 否”的行数比例**，每周筛一次：执行率 < 80% 说明问题不在系统、在执行（第 7 章），先解决执行再谈优化。
