...
8.11-8.12 讲的是“成交在哪里、谁在成交”。Tick 图换一个切法：**按固定成交笔数切 K 线（如每 500/1000 笔一根），与时间无关**——时间图上的“无聊横盘”在 Tick 图上可能是几十根活跃 K 线，时间图上的“大阳线”在 Tick 图上可能只有几笔大单。Tick 图把市场的“心跳频率”摊开：**成交量大的时候 Tick 图走得快、K 线多，成交量小的时候走得慢、K 线少**。Ali 体系在 Tick 图上做了三件事：TX 水平（极值参考位）、CCTK（连续同向 K 线）、TX-D（定向交易信号）。三者回答同一个问题：**失衡有多持续？**
