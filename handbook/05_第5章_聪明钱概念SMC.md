...
*图 5-3 BOS（破前高延续）与 CHoCH（破 HL 转势警告）*
...
![图 5-6 完整 SMC 交易流程：顶部=多级别背景过滤（日线趋势→4H CHoCH→1H 等回调）；中部=五步执行（画结构→等 sweep→找入场区→执行→管理），每步标注本书对应节；底部=关键认知——这套流程就是第 4 章趋势回调系统的 SMC 翻译版](images/fig_p5_smc_flow.png)

*图 5-6 完整 SMC 交易流程：顶部=多级别背景过滤（日线趋势→4H CHoCH→1H 等回调）；中部=五步执行（画结构→等 sweep→找入场区→执行→管理），每步标注本书对应节；底部=关键认知——这套流程就是第 4 章趋势回调系统的 SMC 翻译版*

图 5-6 把 5.2-5.6 的知识点收拢成一个可执行的序列：顶部先问"背景允不允许做"，中部每一步都在回答"现在到了哪一步"，底部提醒你这不是新体系。**五步的顺序不能跳**——先有结构才知道 sweep 有意义；先确认 sweep 才知道 FVG/OB 是入场区而不是下跌中继；先有入场区才有执行和管理。对照表与图 5-6 就是同一件事的两种呈现：表格给你"每个 SMC 术语翻译成什么"，图给你"翻译后的系统长什么样"。
...
![图 5-7 Wyckoff 吸筹结构：PS→SC→AR→ST→Spring→SOS→LPS](images/fig_p41_x187.png)

*图 5-7 Wyckoff 吸筹结构：PS→SC→AR→ST→Spring→SOS→LPS*
...
![图 5-8 Volume Profile：量按价格横向分布——横盘堆积（VA）→ 突破穿过 LVN 快速区 → 新高处再堆积（HVN）](images/fig_p5_volume_profile.png)

*图 5-8 Volume Profile：量按价格横向分布——横盘堆积（VA）→ 突破穿过 LVN 快速区 → 新高处再堆积（HVN）*
...
![图 5-9 摆动范围与溢价/折价：左侧范围 vs 当前范围，中点分界“贵不贵”](images/fig_p5_swing_range.png)

*图 5-9 摆动范围与溢价/折价：左侧范围 vs 当前范围，中点分界“贵不贵”*
...
![图 5-11 S2B：先卖推低扫 SSL，再突破前高做多（B2S 为镜像）](images/fig_p5_s2b_b2s.png)

*图 5-11 S2B：先卖推低扫 SSL，再突破前高做多（B2S 为镜像）*
...
![图 5-12 流动性诱导：亚盘高低点被伦敦/纽约掠夺，内外流动性层层嵌套](images/fig_p5_liquidity_induce.png)

*图 5-12 流动性诱导：亚盘高低点被伦敦/纽约掠夺，内外流动性层层嵌套*
...
5.7 的完整流程把以上收成五步：画结构 → 等 sweep → 找入场区 → 执行 → 管理——本质是第 4 章趋势回调系统的 SMC 翻译版。
