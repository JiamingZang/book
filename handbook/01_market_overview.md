...
**这对你的意义**：图表上的成交量是“冰山一角”——1.1 说的“成交量不完整”不是猜的，是结构性的：大资金越来越多地在你看不见的地方成交（呼应 1.6 的量价注意）。
...
算一笔具体的账：$10,000 账户，1:100 杠杆，下 1 手 EURUSD（名义 $100,000，占保证金 $1,000）。价格反向走 100 pips（$1,000 亏损），账户权益降到 $9,000，保证金比例 = 9,000 / 1,000 = 900%，还安全。但如果价格反向走 500 pips，权益 $5,000，保证金比例 500%——部分平台在 100% 以下强平，也就是说你**在价格反向走约 1% 时就被强平**，而你原本的止损可能在 0.5%（50 pips）——如果你的仓位是按“感觉”而不是按公式下的，止损还没触发，爆仓先来了。
...
![图 1-1R 真实数据：BTC 时段活跃度——上=北京时间各小时平均波动（点），下=各小时总成交量（千 BTC）；美盘（北京 22 时-8 时）占全天成交量 52%、波动峰值在北京 5-6 时——加密的黄金时段与外汇（伦敦-纽约重叠，北京 20-24 时）不重合（Binance 5 分钟，2026-06-29 ~ 08-13）](images/fig_real_ch1_sessions.png)

*图 1-1R 真实数据：BTC 时段活跃度——加密的黄金时段是美盘，不是伦敦-纽约重叠（数据源：Binance BTCUSDT 5m K 线）*

用 BTC 真实数据看时段分布，两个反直觉的结论：**加密的活跃高峰在美盘（北京 22 时-次日 8 时，占全天成交量 52%），而不是外汇意义上的“黄金窗口”**——因为加密的对手盘来自美国时区的科技与量化资金，美股收盘后的 22-8 时才是主战场；**波动峰值在北京 5-6 时（≈谷底 1 时的 2.2 倍）**，亚盘反而有 8 时的一个小高峰。含义：做加密不要拿外汇时段表套，用你自己的历史数据按小时统计“波动 + 成交量”两个维度，画一张自己的时段分布——这就是第 8 章“过滤条件”的量化版。
