...
![图 1-11 外汇要点：左=OTC 无中央账本 → tick 量只是近似，量价方法降级；右=点差是每笔固定成本，频率越高侵蚀越重，exotic 点差大到吃穿窄止损的盈亏比](images/fig_p1_forex_structure.png)

*图 1-11 外汇要点：没有中央账本 → 量只能当参考；没有手续费 → 成本主要是点差，频率越高越致命*
...
![图 1-12 跨市场相关性与风险叠加：左=相关性速查（EURUSD/GBPUSD 高度正相关、DXY/非美货币负相关、ES/NQ 高度正相关、黄金/美元通常负相关但危机时同涨）；右上=风险叠加陷阱（EURUSD 0.5% + GBPUSD 0.5% = 实际 1% 押在“美元走弱”）；右下=三条对策（高相关只选一个或合并算、用 DXY 看方向、组合算总风险）](images/fig_p1_correlation.png)

*图 1-12 跨市场相关性与风险叠加：相关 ≠ 分散——高相关品种押两注，等于同一方向重仓*
...
![图 1-13 A 股规则与脾气：左=规则速查（T+1/涨跌停/时段/集合竞价/最小单位/做空/杠杆，与期货外汇对照）；右上=成本结构波段税（10 万进出一次约 102 元，一年 100 次=年化 10% 成本）；右下=三条差异三条应对（周期放大日线/T+1 止损推迟一天/涨跌停是流动性断层不是信号）](images/fig_p1_ashare.png)

*图 1-13 A 股规则与脾气：同一套价格行为 + 三条特殊规则（T+1/涨跌停/政策驱动）的约束解——先摸清规则再谈技术*
...
![图 1-14 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）](images/fig_p1_asset_allocation.png)

*图 1-14 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）*
