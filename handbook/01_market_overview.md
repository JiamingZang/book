...
![图 1-3R 真实数据：BTC 与 ETH 的相关性不是常数——上=5 分钟收益散点按波动分档着色（青=最平静 0.82 基点/5m，红=最剧烈 16.1 基点/5m）：平静时散点几乎是个圆（r=0.17，两笔独立交易），剧烈时聚成一条直线（r=0.90，同一笔交易的镜像）；下左=各波动档的相关性柱（0.17→0.51→0.78→0.90 单调攀升），下右=每日 24h 滚动相关性（0.63~0.93 波动，全样本 r=0.86）——相关性是波动的函数，不是常数（数据源：Binance BTCUSDT/ETHUSDT 5m K 线）](images/fig_real_ch1_corr.png)

*图 1-3R 真实数据：BTC 与 ETH 的相关性不是常数——平静期 r=0.17，剧烈期 r=0.90（数据源：Binance BTCUSDT/ETHUSDT 5m K 线）*
