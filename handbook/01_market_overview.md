...
> **阅读提示**：本章不是“市场百科”，而是回答一个问题：**钱是怎么被撮合成价格的，规则又怎么决定你能用什么方法？** 先搞清参与者、订单撮合、点差滑点和品种脾气，再谈技术分析。新手建议完整读一遍；有经验可扫读 1.1–1.6 后直接进第 2 章；1.16–1.17 的 A 股与资产配置通识可按需查阅，不影响主线。正文中括号标注的人名（Brooks、Ali、南桥、Tefi 等）是观点/资料来源，不是新术语——每人一节收录在附录 C（大 V 参考）。


大多数交易教程，第一章就教你看 K 线、画趋势线。但这本书想先带你搞清楚一件更根本的事：钱和市场，到底是怎么转的。

为什么？因为技术分析里的每一条规则——“支撑会被测试”“突破要放量”“止损放在结构外”——背后都站着一个机制：真实的人，在真实的约束下，用真金白银买卖。你看到的每根 K 线，都不是随机跳动，而是订单、杠杆、流动性（市场能承接多少单而不把价格打飞——流动性好=买卖盘厚、点差窄）、时段共同作用的结果。
...
| 做市商 | 赚点差 | 同时挂买卖单，提供流动性 | 点差的来源，插针（流动性瞬间枯竭造成的极端长影线，详见 1.6）的制造者之一 |
...
- **每笔交易都站着机构**：你买入时，对面必须有一个机构愿意卖出，而且它必须有一个正的交易者等式（期望值为正的赔率公式——胜率×盈利 > 败率×亏损，第 6.4）才肯接。这意味着：**你以为的“完美交易”，对面是一台算过账的机器**——它认为这笔交易对它有利才接的。
...
**参与者生态的另一种分法（威科夫 2.0——按吸筹/派发周期解读盘面的方法，详见第 5 章；“2.0”指现代整理版本）**：按“入场意图”分，所有参与者只有三类：
...
| 迟缓者 | 16% | 最不知情、被情绪（贪婪/FOMO，Fear of Missing Out 错失恐惧——见术语表）驱动——接最后一棒 |
...
**电子化程度：市场的“机器浓度”**：国际清算银行（BIS）的全球评估——期货 90%、股票 80%、CDS（信用违约互换）80% 的交易电子化；公司债低（投资级 40%、高收益 25%）。美国股票流动性分散在约 88 个来源（交易所+暗池+内部化）：

- 近 **40% 的交易在交易所外进行**；
- 纯暗池交易所占 16-18%；彭博估计加上内部化等“类暗池”场外成交后，暗池相关交易已占总量 30% 以上（两种口径：前者仅指交易所内的暗池）。
...
这条理论的实操含义：**把“市场应该往哪走”换成“市场现在在哪个均衡位附近、离它多远”**——大多数突破都发生在价格偏离均衡位之后的回归途中（呼应 4.10 均值回归、5.4 BOS/CHoCH——Break of Structure 结构破坏（也称结构突破）/ Change of Character 性质变化——的结构逻辑）。
...
不同品种的机制差异，直接决定你的策略和成本。同样的“价格行为方法”，在外汇、期货、加密上的执行完全是三件事。（Prop＝自营交易公司；“考核”＝通过其模拟盘评估后拿真资金分成，详见第 9 章。）
...
**外汇**：全球最大的市场，日均交易量 7.5 万亿美元级别。它是场外市场（OTC），没有中心交易所，所以**没有统一的真实成交量**——你在平台看到的“成交量”是 tick 量（报价变动的次数），只是近似。这意味着依赖量价的方法（Wyckoff、订单流、Volume Profile——三类用成交量与价格分布分析机构行为的方法，分别见第 5 章 Wyckoff 节、8.11 足迹图与订单流、5.12 Volume Profile）在外汇上只能当参考。但外汇也有它的优势：流动性最大、点差小、24 小时交易、做空自由、无涨跌停。对纯价格行为交易者，主流货币对足够干净。
...
| 限价单 | 到指定价或更优才成交 | 无 | 挂单入场（如 pin bar——带长影线的小实体反转 K 线，详见第 3 章——的 50% 位） |
...
**订单镜像思维（Brooks）**：把任何市场先归类成“止损单市场”或“限价单市场”，然后假设**你的每一张订单，对面都站着一个方向相反的对手单**——你在前一根 K 线高点上方 1 tick 挂止损买单，对方很可能在同一个价位挂限价卖单；你用限价单在前高做空，对方很可能用止损单在更高处买。你永远不知道对手是谁，但这不重要：这个假设逼你换位思考“对面为什么愿意接我的单”——我的概率高，他就得有大的回报或小的风险作为补偿（交易者等式，6.4/6.7）。实战含义：① 止损单入场 = 顺着突破方向追，市场至少会朝你移动一个价格变动（概率高、止损远）；② 限价单入场 = 在区间/回调里等价格回来，成交后通常还有反转风险（概率低、止损近）；③ 每次突破（哪怕只是突破前一根 K 线高低点）都值得停下来评估成功或失败的可能性，因为你止损单的对家往往就是那个“知道自己在做什么”的机构。
...
**订单的 8 种用法（威科夫 2.0 的完整矩阵）**：上面 6 种订单类型（含 OCO/OTO 两种组合单）只是“骨架”，真正的用法是 2（买卖）× 2（主动/被动）× 2（入场/离场）的矩阵——同一张订单，站在不同交叉点，作用完全不同：
...
**被动单的另一个性质（威科夫）**：限价单能**阻止**价格移动，但不能**推动**价格移动；**缺乏供应可以促进价格上涨，缺乏需求可以促进价格下跌**——所以“买墙/卖墙消失”往往比“买墙/卖墙出现”更重要：不是有人要买才涨，而是没人肯卖才更容易涨。

**两个实操要点**：① **限价单永不滑点**——这是第 3 章 pin bar“影线 50% 位挂限价单”入场法的隐藏优势（上文订单类型已详述，这里只记住结论）；② **挂单必须设时效**（GTC 长期有效 / DAY 当日有效）。不设时效的挂单是过期炸弹——三天后你忘了它，它突然成交在一个你根本没在看的价位。纪律：当天计划里的单才挂，收盘前检查所有挂单。
...
先想一个问题：**你想下“单笔风险 0.5%”的单，需要哪三个数字才能算出几手？** 答案是：① 价格最小怎么跳（点/Pip），② 跳一下你的账户盈亏多少（点值），③ 一个合约代表多大敞口（合约乘数）。这三个数字不是术语，是仓位公式的三个输入。

- **点（Pip）**：外汇最小报价单位。多数货币对 1 pip = 0.0001（日元对 = 0.01）。比如 EURUSD 从 1.0850 到 1.0851 就是 1 个 pip——它回答“价格移动的最小刻度是什么”。
- **点值（Pip Value）**：1 标准手（100,000 单位）每动 1 pip ≈ $10（美元计价对）。它回答“这一个刻度，换成我的账户金额是多少”——这是把止损距离翻译成钱的桥梁。
- **合约乘数**：期货每点值多少钱，由合约决定。ES 每点 $50，NQ 每点 $20，CL 每点 $1000（详见 1.10）。它回答“一个合约的价格变动，对应多少盈亏”。
...
**保证金占用 = 合约名义价值 ÷ 杠杆**（注意区分两个杠杆：**平台杠杆**=平台允许的上限（1:30/1:100/1:500）；**实际杠杆**=你真正动用的仓位（头寸名义价值 ÷ 账户权益）——决定你爆仓距离的是实际杠杆，不是平台上限）

**示例**：1 标准手 EURUSD ≈ $100,000 名义价值。1:30 杠杆下，占用保证金 ≈ $3,333。也就是说你用 $3,333 撬动了 $100,000 的头寸——价格每动 1%，头寸盈亏 $1,000，相对你的 $3,333 本金就是 1% × 30 = **30%**。这就是杠杆的定义：盈亏被放大了 30 倍（注意方向是"乘"不是"除"——下面的爆仓算例和图 1-1R 都建立在这一步之上）。
...
算一笔具体的账：$10,000 账户，1:100 杠杆，下 1 手 EURUSD（名义 $100,000，占保证金 $1,000）。价格反向走 100 pips（$1,000 亏损），账户权益降到 $9,000，保证金水平 = 9,000 / 1,000 = 900%，还安全。但如果价格反向走 500 pips，权益 $5,000，保证金水平 500%——部分平台在保证金水平跌破维持线（通常为初始保证金的一半）时强平：1 手的情形要反向约 9.5%（约 950 pips）才爆仓；可如果你按“感觉”把仓位下到 10 手（1:100 满仓，名义 $1,000,000），反向约 50 pips（≈0.5%）时保证金水平就触及维持线被强平——而你原本的止损可能就设在 0.5%（50 pips）——**止损还没触发（或刚触发就被强平），爆仓先来了**。
...
上面的算例是抽象的，把它放到一根真实 K 线上看——**同样的追高错误，实际杠杆只决定你死在哪一步**：

![图 1-1R 真实数据：同样的追高，不同的杠杆，不同的死法——2026-06-30 日高 60684（01:30）后 01:35 追多入场 60501（V=200，FOMO）；此后阴跌 -3.8% 到 23:05 日低 58201，四档实际杠杆的爆仓线（维持线口径：爆仓距离 = 0.5 / 实际杠杆）：100x 线 60199（-0.5%）→ 02:50 触及爆仓（入场后 75 分钟）；50x 线 59896（-1.0%）→ 08:30 触及爆仓（V=143）；20x 线 58989（-2.5%）→ 20:20 放量触及爆仓（V=502）；10x 线 57476（-5.0%）→ 全天未触及（日低 -3.8%），但账户浮亏 -38%——同样的判断错误，杠杆只决定你死在哪一步（数据源：Binance BTCUSDT 5m K 线）](images/fig_real_ch1_liquidation.png)

*图 1-1R 真实数据：同样的追高，不同的杠杆，不同的死法——100x 死于 02:50，10x 活到收盘但浮亏 -38%（数据源：Binance BTCUSDT 5m K 线）*

读这张图看三件事：

**① 杠杆不改变你的判断，改变的是爆仓距离。** 四个杠杆的持仓方向、入场点、止损想法完全一样——都在 01:35 追多 60501，都觉得"会继续涨"。区别只在爆仓线离入场多远：100x 的线在 60199（-0.5%），02:50 一根普通阴线就击穿；50x 在 59896（-1.0%），08:30 击穿；20x 在 58989（-2.5%），20:20 放量击穿；10x 在 57476（-5.0%），全天没碰到。**你"觉得"这单会涨的判断，四种杠杆一模一样——实际杠杆只是决定了你在 -0.5% 还是 -5% 的地方被强制出局。**

**② 爆仓先于止损——这是高杠杆最阴险的地方。** 假设你是个守纪律的人，入场时挂了止损。20x 的爆仓线 58989 在 20:20 被击穿（V=502 放量）；如果你把止损设在日低 58201 下方（-3.8%，给足空间"别被扫"）——但价格直到 23:05 才最低触及 58201，设在它下方的止损**根本没轮到触发**，20:20 爆仓先来（比日低出现早约 2 小时 45 分）——**你根本没机会执行那个止损，平台先替你平了**。这正是 1.5 正文算例说的"止损还没触发（或刚触发就被强平），爆仓先来了"。

**③ "没爆"不等于"没伤"。** 10x 是四个杠杆里唯一活到收盘的，但日低 -3.8% × 10x = **-38% 账户权益**——一次追高错误就亏掉近四成账户（呼应 6.3 连亏与回撤：-38% 需要 +61% 才能回本）。真实的安全不是"选个不爆的杠杆"，而是用第 6 章仓位公式反推出**这个错误发生时你能承受多少亏损**，再反推需要的杠杆——平台给你 1:500 不是让你用的，是让你更容易爆的。
...
左图是你每笔交易的真实处境：两个价格同时存在，你买入永远成交在更高的 Ask、卖出永远成交在更低的 Bid，一进一出之间点差已经亏掉。这就是为什么 7.8 的 Brooks 清单里“剥头皮”类交易要额外警惕——进出次数越多，点差成本吃掉的利润越多。右图把“跳空造成滑点”这一最常见情形画成了具体数字：止损单是跟着价格走的，价格跳空时你只能在新的价位成交。**结论一句话：成本意识要进入每笔交易的决策——选流动性好的时段、用限价单锁成本（1.6）、远离数据瞬间**。
...
**伦敦-纽约重叠（北京 20:00-24:00）是黄金窗口**：流动性最好、点差最小、大行情高发。第 5 章 SMC（Smart Money Concepts，聪明钱概念）的 Kill Zone（机构集中交易时段）指的就是这个。两个全球最大的金融中心同时开市，资金量最大，趋势最容易启动，信号也最可靠。
...
**冬令时（11 月-次年 3 月）所有时间 +1 小时**：上表是美国夏令时（3 月-11 月）口径；进入冬令时，伦敦时段变为北京 16:00-次日 1:00，纽约时段变为北京 21:00-次日 5:00，伦敦-纽约重叠黄金窗口相应移到北京 21:00-次日 1:00（美股 9:30 开盘则对应北京 22:30）。做外汇/期货的人必须每年调两次这个"时差表"，写进你的交易笔记。
...
![图 1-2R 真实数据：BTC 时段活跃度——上=北京时间各小时平均波动（点），下=各小时总成交量（千 BTC）；美盘（北京 22 时-7 时）占全天成交量 52%、波动峰值在北京 6 时（成交量峰值在北京 5 时）——加密的黄金时段与外汇（伦敦-纽约重叠，北京 20-24 时）不重合（Binance 5 分钟，2026-06-29 ~ 08-13）](images/fig_real_ch1_sessions.png)

*图 1-2R 真实数据：BTC 时段活跃度——加密的黄金时段是美盘，不是伦敦-纽约重叠（数据源：Binance BTCUSDT 5m K 线）*
...
| CPI 通胀 | 各国货币、债券 | 月度 |
| GDP | 各国货币、债券 | 季度初值与修正 |
| 期权到期日 | 期权及标的（三巫日/四巫日——指数期权、个股期权与期货集中同一天到期的日子——波动放大） | 每季度第三个周五（3/6/9/12 月）；普通月度期权每月第三个周五到期；期货/加密另有月度与季度到期 |
...
> **期权到期日例外**：美股每季度第三个周五（triple/quadruple witching，3/6/9/12 月；普通月度期权是每月第三个周五到期）的波动不是信息驱动的，而是到期平仓/展期驱动的——那天成交量放大、价格行为容易失真，信号质量同样下降，纪律与数据日相同。
...
**新闻已经在价格里（Al Brooks）**：你看到新闻时，机构通常已经知道了。大多数机构交易者在新闻发布前就了解它——它们已经在按“明天的新闻”交易了。所以：
...
- **止损宽度要按品种的波动率（ATR，Average True Range，平均真实波幅——第 6 章的止损计算工具）调**，不是固定点数。EURUSD 的 50 点和 ES 的 50 点完全是两个世界——ES 一个交易日轻松波动 100+ 点，50 点的止损在 ES 上等于没设。
...
![图 1-10 品种脾气：波动尺度 × 活跃时段 × 成本结构 × 量是否真实——同一套价格行为，在不同品种上执行细节完全不同](images/fig_p1_temperament.png)

*图 1-10 品种脾气：先摸清脾气，再谈技术*
...
- **展期日（rollover day，Ali）**：换月那天新旧两个合约同时在交易——**旧合约的成交量更高（成交量次日才转移到新合约）**；由于两者都在交易，两张图都呈现有效形态（无跳空缺口），但形态并非总是一致。操作建议：**同时观察两张图表，但优先交易成交量更大的旧合约**——成交量小的一方 K 线形态容易失真（低流动性下的价格不代表真实市场）；旧合约的形态才更准确反映市场真实走向。这也解释了为什么换月前一周开始，主力资金逐渐向新合约迁移、旧合约波动被放大。
...
![图 1-11 期货合约规格速查：ES/NQ/YM/CL/GC 每点价值与 tick + 四个核心机制（合约规格/tick 价值/到期换月/保证金）+ 新手从 ES 起步](images/fig_p1_futures_spec.png)

*图 1-11 期货合约规格速查：ES/NQ/YM/CL/GC 每点价值与 tick + 四个核心机制（合约规格/tick 价值/到期换月/保证金）+ 新手从 ES 起步*

### 1.11 股票：做空限制与规则

**核心机制**：

- **做空限制**：股票做空要“借券”，不是所有股票都能做空；且有卖空限制（美国现行为 Rule 201：单日跌幅达 10% 后才触发，限制对该股票追加卖空；并非“价格下跌时一律不能追空”）。外汇/期货做空更自由。如果你主要做空，股票不是好战场。
...
![图 1-12 股票做空：先借券才能卖，Rule 201 是“触发式限制追加卖空”而不是做空禁令——结构性约束决定它不适合做空主线](images/fig_p1_stock_short.png)

*图 1-12 股票做空：借券门槛 + 触发式卖空限制*

### 1.12 外汇：要点速查

（本章 1.2 已概览，这里是速查）

- **24h 交易（周一~周六），无集中交易所，OTC 市场**——没有中央撮合，意味着“谁在成交”没有统一账本，这是下面量不真实、点差为成本的根源。
- **量不真实**：无统一成交量，用 tick 量近似——量价方法在外汇上只能当参考。机制：OTC 市场同一笔价格由无数做市商各自撮合，没有像期货那样的中央交易所把每一笔成交记在同一本账上；你看到的 tick 量只是“报价跳动次数”，不是真实成交吨位。
- **点差为主要成本**：外汇没有交易所手续费，交易成本主要是买卖价差；伦敦-纽约重叠时段流动性最好、点差最窄，是因为两个最大交易中心同时开市，做市商之间的竞争把价差压薄。点差是每笔固定成本，频率越高侵蚀越重——这直接进入第 6 章的期望值公式：同一套信号，刮头皮比波段对点差更敏感。
- **隔夜利息（swap）**：持仓过夜有正负利息。高息货币对的多单收利息（正 swap），低息货币对的多单付利息（负 swap）。机制：这是两国利率差的直接转账——你持有高息货币、卖出低息货币，等于赚取利差；方向反了就付利差。长期持有要算这笔账，它进第 6 章的“成本”项。
- **货币对分类**：主流对（EURUSD/GBPUSD/USDJPY，点差小）、交叉对（EURGBP，点差大）、exotic（点差极大，别碰）。机制：点差大小由流动性决定——主流对是全球交易量最大的货币对，做市商竞争激烈；exotic 交易量小、做市商要覆盖更大风险，所以点差大。别碰 exotic 不只是“贵”，而是点差大到足以把窄止损的盈亏比吃穿：止损越近，固定成本占风险的比例越高。
- **对价格行为的适配**：主流对流动性好、结构清晰，适合价格行为；但量价确认弱于期货。原因同上：没有真实量，量价信号在期货上才是完整的。

![图 1-13 外汇要点：左=OTC 无中央账本 → tick 量只是近似，量价方法降级；右=点差是每笔固定成本，频率越高侵蚀越重，exotic 点差大到吃穿窄止损的盈亏比](images/fig_p1_forex_structure.png)

*图 1-13 外汇要点：没有中央账本 → 量只能当参考；没有手续费 → 成本主要是点差，频率越高越致命*
...
- **先分清“现货 vs 合约”**：现货（spot）= 直接买卖真实币，你持有的是币本身；合约 = 保证金交易，你持有的是“价格涨跌”的敞口——永续合约（perpetual）没有到期日、可一直持有，但有多空双向、杠杆与资金费率机制。本书的价格行为信号在两个市场都适用；仓位公式（第 6 章）只认你实际承担的风险，杠杆越高仓位越小。
...
![图 1-3R 真实数据：BTC vs ETH 波动率对比——上=归一化价格同涨同跌；下=24h 滚动波动率（日均 1σ %），恐慌时飙升、平静时压缩（BTC/ETH 5 分钟，2026-06-29 ~ 08-13）](images/fig_real_ch1_vol.png)

*图 1-3R 真实数据：BTC vs ETH 波动率对比（数据源：Binance BTCUSDT/ETHUSDT 5m K 线）*
...
“日内 5-10% 常见”不是抽象数字，下面这张图就是它的一次真实演出——**下跌趋势里的诱多尖峰**：

![图 1-4R 真实数据：下跌趋势中的尖峰陷阱 → 扫损 → 加速下跌——18:40 高点 65410 持续阴跌到 21:25 低点 64947；21:30-21:45 连续 3 根放量大阳尖峰冲到 65718（+1.2%，追多者入场）；21:50 第一根反向大阴拒绝；22:40 跌破尖峰起点扫损（-1.2%）；01:35 回测尖峰起点=逃命点；06:40 放量破位；09:05 最低 63059，距尖峰顶 -4.05%（BTCUSDT 5m，2026-07-27 18:30 ~ 07-28 09:30）](images/fig_real_ch1_spike_sweep.png)

*图 1-4R 真实数据：下跌趋势中的尖峰陷阱 → 扫损 → 加速下跌（数据源：Binance BTCUSDT 5m K 线）*

**这张图要读出三件事**：

1. **下跌趋势里的放量大阳尖峰，常常是“诱多”而不是“反转”**。18:40 到 21:25 市场一直在阴跌，21:30 突然三根放量大阳（V≈110-200，常态量 30-70）冲高 +1.2%——看起来很强势，但它是下跌途中的反弹，不是趋势反转（呼应 2.11 尖峰 5 项识别标准：大实体、无重叠、创新高它全占，但它缺的是趋势背景）。尖峰测的不是“买方多强”，而是“卖方有没有出现”——21:50 第一根反向大阴就是卖方出现的信号：尖峰结束，追高者此时唯一正确的动作是离场。
2. **跌破尖峰起点 = 追高者的止损全部被扫**。22:40 价格跌破 64947（尖峰起点），所有在尖峰买入的人浮亏（从尖峰顶 65718 算已 -1.2%），止损盘被打掉后价格反而止跌回测——01:35 反弹到 65090（尖峰起点附近）就是标准的**逃命点**：被套的多头趁反弹走人，新空头则在这里进场（呼应 5.6 sweep：扫掉止损后回测确认）。
3. **加密的“快”是最大特征**。从尖峰顶 65718 到最低 63059，**只用了 12 小时，-4.05%**——这就是 1.13“日内 5-10% 常见”的一个真实实例。同样的尖峰-扫损剧本在外汇/股指上可能 2-3 天走完，在加密上半天就走完了。**信号一样，节奏快一个量级——止损必须更紧、仓位必须更小，否则一个“半小时的误判”就是 -4%。**

**怎么把这张图变成规则**：下跌趋势中出现放量大阳尖峰 → 不追多，等卖方信号（第一根反向 K 线）或等回测；尖峰后第一根反向 K 线 = 追高者离场信号；跌破尖峰起点后任何反弹回该位置 = 逃命点/新空头入场点，不是反转点。
...
- 黄金和美元通常负相关（美元强则金价弱）；**极端风险事件中要区分两种情形**：避险情绪下美元与黄金可能同涨（都被当避险买），但流动性危机时也可能“一切皆跌”（黄金也被抛售换现金）。
...
![图 1-14 跨市场相关性与风险叠加：左=相关性速查（EURUSD/GBPUSD 高度正相关、DXY/非美货币负相关、ES/NQ 高度正相关、黄金/美元通常负相关但危机时同涨）；右上=风险叠加陷阱（EURUSD 0.5% + GBPUSD 0.5% = 实际 1% 押在“美元走弱”）；右下=三条对策（高相关只选一个或合并算、用 DXY 看方向、组合算总风险）](images/fig_p1_correlation.png)

*图 1-14 跨市场相关性与风险叠加：相关 ≠ 分散——高相关品种押两注，等于同一方向重仓*

**上面是合成速查，下面用真实数据把"相关 ≠ 分散"实证一遍**——BTC 和 ETH 同一个 5 分钟数据（2026-06-29 ~ 08-13，共 12959 根），按 BTC 5 分钟收益的波动大小分成四档：

![图 1-5R 真实数据：BTC 与 ETH 的相关性不是常数——上=5 分钟收益散点按波动分档着色（青=最平静 0.82 基点/5m，红=最剧烈 16.1 基点/5m）：平静时散点几乎是个圆（r=0.17，两笔独立交易），剧烈时聚成一条直线（r=0.90，同一笔交易的镜像）；下左=各波动档的相关性柱（0.17→0.51→0.78→0.90 单调攀升），下右=每日 24h 滚动相关性（0.63~0.93 波动，全样本 r=0.86）——相关性是波动的函数，不是常数（数据源：Binance BTCUSDT/ETHUSDT 5m K 线）](images/fig_real_ch1_corr.png)

*图 1-5R 真实数据：BTC 与 ETH 的相关性不是常数——平静期 r=0.17，剧烈期 r=0.90（数据源：Binance BTCUSDT/ETHUSDT 5m K 线）*

**这张图要读出三件事**：

1. **相关性不是常数，是波动的函数**——最平静的 1/4 时间里 BTC 和 ETH 几乎各走各的（r=0.17），最剧烈的 1/4 时间里同步到 r=0.90。真实世界的相关性是动态的：不是"同涨同跌"这种静态印象，而是"波动越小越独立、波动越大越同步"。
2. **"分散"是平静期的错觉**——你在 7 月上旬的平静期同时做多 BTC 和 ETH，看走势像两笔独立交易；但 7 月中旬波动一来，0.9 的相关性让"两笔"瞬间变成"一笔×2 倍仓位"。这就是 1.14 风险叠加陷阱的真实版本：**你以为是分散，其实是重仓**。
3. **最需要分散的时候恰恰最不分散**——剧烈波动期（正是你最容易亏大钱的时候）相关性飙向 0.9，这就是"流动性危机时一切皆跌"的数学形式。所以 1.14 的对策不是"再多买几个加密品种"，而是跨资产类别（加密 + 股指 + 债券）分散，或者组合层面做相关性修正。
...
![图 1-15 选品种决策树：目标（prop/实盘）→ 品种 → 再匹配周期、方法、波动承受；新手先聚焦单一品种深耕](images/fig_p1_choose_instrument.png)

*图 1-15 选品种决策：目标 → 周期 → 方法 → 波动承受*
...
| 做空 | 融券受限、门槛高 | 借券 + 触发式卖空限制（Rule 201，单日跌 10% 后触发） | 自由 |
...
![图 1-16 A 股规则与脾气：左=规则速查（T+1/涨跌停/时段/集合竞价/最小单位/做空/杠杆，与期货外汇对照）；右上=成本结构波段税（10 万进出一次约 102 元，一年 100 次=年化 10% 成本）；右下=三条差异三条应对（周期放大日线/T+1 止损推迟一天/涨跌停是流动性断层不是信号）](images/fig_p1_ashare.png)

*图 1-16 A 股规则与脾气：同一套价格行为 + 三条特殊规则（T+1/涨跌停/政策驱动）的约束解——先摸清规则再谈技术*
...
![图 1-17 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）](images/fig_p1_asset_allocation.png)

*图 1-17 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）*

（左）总资产的两条腿：交易账户只占 15%（≤10-20%）——它的职责是验证能力、积累经验，亏光了不影响生活；配置账户占 85%——它负责长期复利，是“输得起”的底气。（右）配置账户内部示例：60% 股票指数基金（增长引擎）+ 30% 债券/货币基金（压舱石）+ 10% 现金（应急）；每年做一次再平衡（卖涨的、买跌的），把比例调回目标——这里的目标即经典的“60/40 口径”：60% 股票、40% 债券+现金（上图的 30% 债券 + 10% 现金就是这 40% 的拆法）。再平衡本身就是一个“强制低买高卖”的纪律。
...
5. 什么时候是日内黄金时段？为什么数据事件前不开仓？

**答案**：① 外汇是 OTC 场外市场、没有中心交易所（1.2）——依赖量价的方法（Wyckoff/订单流）在期货上更正统，外汇上只能参考；② 杠杆不改变胜率，放大的是仓位误差（1.5）——同样的判断错误，低杠杆是小伤、高杠杆直接爆仓；③ T+1（1.16）——当天买的仓位当天不能卖，止损最早次日生效，所以信号周期要放大到日线以上、仓位按两天的波动来算；④ 买入成交在 Ask、卖出成交在 Bid（图 1-7）——一进一出天然亏一个点差，高频进出（剥头皮）对点差最敏感（1.6）；止损单会滑点（跳空时以更差价格成交），限价单永不滑点（1.3）；⑤ 外汇口径：伦敦-纽约重叠时段（北京 20:00-24:00，冬令时 +1 小时，1.7）流动性最厚、点差最窄——亚盘信号质量低；加密的黄金时段是美盘（北京 22:00-次日 7:00，图 1-2R），别照搬外汇时段表；数据事件是技术面的黑天鹅（1.8），公布瞬间价格可能跳空扫掉止损，数据前 15 分钟不开新仓。
