...
![图 1-11 A 股规则与脾气：左=规则速查（T+1/涨跌停/时段/集合竞价/最小单位/做空/杠杆，与期货外汇对照）；右上=成本结构波段税（10 万进出一次约 102 元，一年 100 次=年化 10% 成本）；右下=三条差异三条应对（周期放大日线/T+1 止损推迟一天/涨跌停是流动性断层不是信号）](images/fig_p1_ashare.png)

*图 1-11 A 股规则与脾气：同一套价格行为 + 三条特殊规则（T+1/涨跌停/政策驱动）的约束解——先摸清规则再谈技术*
...
![图 1-12 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）](images/fig_p1_asset_allocation.png)

*图 1-12 资产配置两条腿：交易账户（≤10-20%）进攻 + 配置账户防守；配置账户内部 60/40 再平衡（编者示例比例）*
