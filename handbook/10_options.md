...
3. **VRP 是长期常态，但恐慌期会反转**。全历史 78.1% 的交易日 IV>RV（平均 +2.5pct）——“担心”长期被高估，卖方长期收保险费；但本窗口仅 58% 为正，7 月 RV 连续 3 周反超（最深 −8.6pct）。**卖方别忘了尾部风险**（呼应 6.3 / 7.1）：收常态溢价，亏恐慌期一次性反超。
...
**价差结构的三个工程规则（期权交易员卡片）**：① **价差距离 ≥ 成本的 2 倍，最好 3 倍**——一句话说，只付价差距离的一半以内、最好只付三分之一（比如 25 美元的价差，支付不超过 12.5 美元）；必要时买一点虚值期权来凑出 3:1 比例；② **用限价单买，不市价追**；如果报价高到价差的一半，说明市场波动太大、这个价差对当前市场太紧——放弃或放宽；③ **入场时机决定工具**：行情刚开始时买期权（便宜、爆发力足），行情已经启动、入场较晚时买价差（此时期权价格已高，卖出腿能对冲掉一部分成本）。
...
- **价差距离 ≥ 交易成本的 2-3 倍**：价差宽度至少是交易成本的 2 倍、最好 3 倍（最多支付价差距离的一半、最好约三分之一）；如果报价已高到价差宽度的一半，说明市场波动过大、这个价差对市场来说过紧；
