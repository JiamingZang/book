...
![图 10-6 期权策略决策树：先问"你想干什么"，再对号入座——怕仓位出事→保护性 Put；持仓收租→备兑 Call；有方向锁风险→垂直价差；赌大波动不赌方向→跨式](images/fig_p10_strategy_tree.png)

*图 10-6 期权策略决策树：先问"你想干什么"，再对号入座——怕仓位出事→保护性 Put；持仓收租→备兑 Call；有方向锁风险→垂直价差；赌大波动不赌方向→跨式*
