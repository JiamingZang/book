...
> **阅读提示**：本章不是策略大全，而是回答一个问题：**从“你要解决什么问题”出发，怎么推出该用哪种期权工具？** 期权把你从“方向交易”带到三维——价格、时间、波动率。先读 10.1-10.3（期权在交易什么、一个具体例子、价格构成：内在价值 + 时间价值），再按需查阅策略表；10.5 讲波动率（含波动率风险溢价）与已有工具的衔接，想进阶可重点看。
...
先别管“期权”这个名字。假设你手里有一笔浮盈的趋势多单（第 4 章那种），你怕它跌回去，又不想现在就卖——你最想要的东西是：**涨了继续归我，跌了有人帮我兜底**。期权就是被这个具体问题逼出来的发明：它不是“更复杂的股票”，而是一张可以把“方向、幅度、时间”拆开单独定价的合约。

做方向交易时，你的盈亏只取决于一件事：价格往哪走、走多少。你买多，涨了就赚，跌了就亏，关系是线性的、直接的——这也是前面九章所有内容共同的前提。期权把这件事变复杂了，因为它让你同时押注三件事：
...
- **合约乘数：权利金要先换算成美元**：上面例子里的“30 点”不是“30 美元”。ES 期权 1 点 = $50，所以 30 点权利金 = $1,500/张；50ETF 期权 1 张对应 10,000 份，权利金 0.0500 元/份 = $500/张（按汇率折算）。下单前先查合约乘数，否则你算不清最大亏损到底是多少钱。
...
上面例子里那个 30 点权利金，到底是怎么定的？假设你是第一个给这种权利定价的人，你会立刻发现：这笔权利至少值两样东西——**现在立刻兑现能赚多少**，和**到期前还可能发生好事的那部分想象空间**。它由两部分组成：
...
**别忘了买卖价差**：期权的真实交易成本不只是权利金，还有买卖价差——越虚值、越临近到期、越冷门的期权，价差越宽。做价差策略时，两条腿的价差成本都要算进去；市价追单往往会把本来成立的盈亏比破坏掉（呼应 10.6“价差三工程规则”的限价单纪律）。

**看跌-看涨平价（Put-Call Parity）：三个价格互相锁死**：同到期、同行权价的 Call、Put 和标的三者之间存在一个数学约束：**Call − Put = 标的价 − 行权价**（忽略利率/分红）。这不是理论装饰，而是套利力量保证的“三角关系”——如果三者不一致，交易者可以买入便宜的组合、卖出贵的组合，无风险锁住价差。对普通交易者的价值：① 检查你看到的期权报价有没有明显错价；② 理解垂直价差为什么能锁死风险——因为 Call/Put/标的是同一件事的三种包装，组合后风险被数学抵消；③ 做市商用这个关系给两边定价，所以 Call 和 Put 的 IV 不是各自独立，而是被平价关系绑在一起。

![图 10-3 看跌-看涨平价：Call、Put、标的是同一件事的三种包装，三者价格被套利锁死——可检查错价、理解价差锁风险、IV 不是各自独立](images/fig_p10_put_call_parity.png)

*图 10-3 看跌-看涨平价：三个价格互相锁死*

直观地看，时间价值衰减不是匀速的（见图 10-4）。

> **要点**：最后两三周 Theta 咬得最狠——同样的方向判断，同样涨了 10 点，离到期远的时候期权还能涨不少，离到期近的时候可能纹丝不动。这就是为什么“买期权做方向”必须同时选对“到期日”。

![图 10-4 时间价值衰减：越接近到期蒸发越快，最后 2-3 周加速](images/fig_p10_theta.png)

*图 10-4 时间价值衰减：越接近到期蒸发越快，最后 2-3 周加速——同样的判断、同样涨 10 点，临近到期可能纹丝不动*

![图 10-5 期权价格构成：左=到期日只剩内在价值（时间价值=0）；右=到期前价格=内在价值（地板）+时间价值（溢价帽，ATM 最高）——同一张图的两个时刻](images/fig_p10_price_breakdown.png)

*图 10-5 期权价格构成：左=到期日只剩内在价值；右=到期前价格=内在价值（地板）+时间价值（溢价帽，ATM 最高）*

图 10-5 把“期权价格 = 内在价值 + 时间价值”画成同一张图的两个时刻：
...
先别管“希腊字母”这个名字。假设你刚发明了期权，现在必须回答四个问题：**标的一动我赚多少？这个“赚多少”自己会变多快？时间每过一天我要付多少租金？波动率一变我又受多大影响？**——这四个问题就是 Delta、Gamma、Theta、Vega。它们不是数学家塞给你的公式，而是任何持有期权的人都绕不开的四个“我要不要担心”的问题。我们逐个用直觉理解，重点是每个在回答什么、对你意味着什么，不是背公式。
...
![图 10-6 Delta 的 S 曲线与 Gamma 的钟形：两者都在平值附近最敏感](images/fig_p10_greek.png)

*图 10-6 Delta 的 S 曲线与 Gamma 的钟形：两者都在平值附近最敏感（合成示意）*
...
**实际用法**：把四个希腊字母落回决策——想表达方向，看 Delta；想判断“买临近到期的平值期权是不是在赌博”，看 Gamma；想判断“事件前买期权贵不贵”，看 Vega/IV；想判断“这笔观点还能不能拿”，看 Theta。四个问题分别对应“方向、速度、波动、时间”。

### 10.5 波动率（IV）：期权的灵魂，也是和你已有知识的连接点

如果说只能懂一个期权概念，那应该是隐含波动率（IV）。它是期权定价的核心，也是期权和你已有知识体系最自然的接口。

**IV 是什么**：它不是技术指标，而是市场对“未来会有多颠簸”的集体报价。把期权理解成一份保险，IV 就是这份保险的费率——IV 高 = 市场认为未来波动大，所以保费贵；IV 低 = 市场认为风平浪静，保费便宜。你买期权时付的权利金，大部分就是在为这份“不确定性保险”付费；这也是为什么“低 IV 买、高 IV 卖”不是玄学，而是“别在保险最贵的时候买保险”。
...
怎么判断 IV 贵不贵？看 **IV Rank / IV Percentile**——把过去一段时间（比如一年）每天的 IV 从低到高排队，看今天的 IV 排在什么位置。排在 80 分位 = 过去 80% 的时间 IV 比现在便宜；排在 20 分位 = 过去 80% 的时间 IV 比现在贵。所以 **>50 = 高于历史中位（波动率偏贵，倾向卖方），<50 = 低于历史中位（波动率偏便宜，倾向买方）**。严格说 IV Rank 有时指“(现 IV−低)/(高−低)”的区间位置，IV Percentile 指历史百分位；两者数值接近但不等价，本书统一按百分位用，规则书里写死一种即可。
...
**IV 的数字怎么翻译成行情尺度（Natenberg）**：波动率与时间平方根成正比——年化波动率除以 √252≈16 近似得到日波动，除以 √52≈7.2 得到周波动。例：IV 30% 的标的，日 1σ≈30%/16≈1.9%，周 1σ≈30%/7.2≈4.2%，月 1σ≈30%/√12≈8.7%。再配合 1σ/2σ/3σ 的 68/95/99.7 概率，你就能把“IV 高不高”翻译成具体问题：目标位离现价几个 sigma？到期前达到的概率大概多少？IV 20 vs 40 不是“贵一点”，而是同样的目标位从约 3σ 变成约 1.5σ——后者被达到的概率完全不同。

![图 10-7 波动率平方根换算：IV 10%-60% 对应的日/周/月 1σ——把 IV 从抽象百分比翻译成行情尺度，判断目标位离现价几个 sigma](images/fig_p10_sigma_time.png)

*图 10-7 波动率平方根换算：IV 不是抽象百分比，是可以用日/周/月 1σ 丈量的行情尺度*
...
**波动率圆锥与均值回复（Natenberg）**：波动率与价格不同——价格可以长期不回到原水平，**波动率却几乎总是向一个均衡区间回复**。Natenberg 用“波动率圆锥”展示：估计区间拉得越长，百分位线越向均值聚拢、均值越稳定。这对交易的含义是：**高 IV 不是“这次不一样”，而是均值回复的起点；低 IV 也不是“永远低波动”，而是买期权相对便宜的时候**。本书 10-3R/10-6R 的恐慌尖峰全部回落，就是波动率圆锥在真实数据上的样子。预测波动率时，Natenberg 还建议给近期数据更高权重——对短期期权尤其如此，因为短期波动率更由“最近发生了什么”决定。注意：均值回复不等于“马上回归”——波动率本身也有趋势性，可能连续走低或连续走高，所以 IV Rank 给的是位置，不是精确的转折时点。
...
![图 10-8 期权四策略到期损益：保护性 Put / 备兑 Call / 牛市价差 / 跨式——损益形状一看就懂（ES 例子）](images/fig_p10_strategies.png)

*图 10-8 期权四策略到期损益：保护性 Put / 备兑 Call / 牛市价差 / 跨式——损益形状一看就懂（ES 例子）*
...
**进阶：波动率价差（Natenberg 思路）**：当你的关注点从“方向”变成“波动率”后，还可以做波动率价差——不赌 IV 的绝对值，而赌不同行权价、不同到期日之间的 IV 相对关系（比如做多某一行权价的 IV、同时做空另一行权价的 IV）。一个最直接可执行的例子是**日历价差**：买远月、卖近月（同一行权价）。它的两个核心性质来自 Natenberg——**多头日历价差希望标的不动**（标的一动，近月 Gamma 的短期损失会吃掉时间差收益），**且受益于 IV 上升**（远月 Vega 通常大于近月，IV 上涨时远月腿升值更多）。所以多头日历适合“事件前 IV 低、你预期波动率要涨但标的方向不明”的场景；空头日历镜像，适合高 IV 后赌波动回落。这超出本书主线，只提示存在：如果你发现自己开始关心“哪条腿 IV 被高估/低估”，说明你已进入 Natenberg 的领域，可回 10.9 的进阶参考继续深入。

![图 10-9 多头日历价差损益：标的不动 = 最大盈利，且受益于 IV 上升；价格移动越远，近月 Gamma 损耗吃掉时间差收益——事件前低 IV 时多头日历，高 IV 后空头日历](images/fig_p10_calendar_spread.png)

*图 10-9 日历价差：多头希望标的不动 + IV 上升，空头镜像*

![图 10-10 期权策略决策树：先问“你想干什么”，再对号入座——怕仓位出事→保护性 Put；持仓收租→备兑 Call；有方向锁风险→垂直价差；赌大波动不赌方向→跨式](images/fig_p10_strategy_tree.png)

*图 10-10 期权策略决策树：先问“你想干什么”，再对号入座——怕仓位出事→保护性 Put；持仓收租→备兑 Call；有方向锁风险→垂直价差；赌大波动不赌方向→跨式*

图 10-10 就是 10.6 那张总结表的图形化：**先问“你想干什么”，再对号入座选策略**。四个分支的箭头都从同一个根出发——因为期权策略不是“哪个更好”，而是“哪个匹配你现在的问题”。读这张图的正确顺序是从上往下：先定用途（根节点），再选形状（中间框），最后检查前提（每框最后一行）——前提不满足就回到根重新选，而不是硬用。**用这张图替代记忆**：背不下四个策略的名字没关系，记得四个问题（怕出事？想收租？有方向？赌波动？）就够了，打开图 10-10 对号入座。
...
![图 10-11 期权放回全书坐标：观点来自价格行为（中心），期权负责把方向、风险、波动率、时间四个维度变成可管理的工具](images/fig_p10_options_coord.png)

*图 10-11 期权放回全书坐标：期权不改变观点，只改变观点的成本、风险与收益形状*

### 10.8 期权的陷阱，以及什么时候该碰、什么时候别碰

期权表面诱人——“杠杆高、亏损有限”——但它的坑不是市场故意设的，而是你从“只赌方向”变成“同时赌方向、波动、时间”之后，**多出来的每个维度都在向你收费**。把坑讲明白，你才知道何时该远离：
...
- **Gamma 效应（到期周的“磁吸+放大”）**：临近到期，大量期权集中在某一行权价（如 0DTE——Zero Days To Expiration，当天到期——的整数关口），做市商为对冲 Gamma 需要买卖标的（买入/卖出标的压 delta），会把价格“吸”向高持仓量行权价、并在到期时放大波动——这就是为什么**到期日当天价格行为失真、假突破频发**；
...
![图 10-12 Brooks 的期权极简主义：左=极简工具树（价格行为 → Call/Put/价差）；右=隔夜用期权锁死跳空风险，入场判断完全来自价格行为](images/fig_p10_brooks_options.png)

*图 10-12 Brooks 的期权观：理解可以深，工具必须简单——注意力留给图表*
...
**进阶参考**：若想系统补期权定价与波动率理论，Natenberg《期权波动率与定价》（Option Volatility & Pricing）是经典入门+进阶参考；本书只取其中与价格行为交易者最相关的一小部分（10.3 定价、10.5 波动率、10.8 陷阱）。想继续深入，可优先看该书第 4 章波动率、第 8 章波动率价差、第 14 章再论波动率、第 16 章市场间价差（16.2 波动率关系）、第 18 章模型与真实世界（18.7 波动率倾斜）。
...
- **保护性看跌期权的有效期约为其期限的 20%**：时间价值衰减不是匀速的（10.3），越接近到期 Theta 咬得越狠。长期 Put 买来时的“保护能力”来自它剩余的时间价值——但过了约 1/5 期限后，每天的时间损耗开始显著吃掉这份保护的成本效益：你为保护付的保费越来越贵、能换到的保护时间越来越短。所以到 20% 时间点要么平掉、要么展期到更远期限，而不是“买了就放着”。若到 20% 时已开始大跌，则保护正在兑现，不机械平仓，先看保护是否仍被需要。
- **价差距离 ≥ 交易成本的 2-3 倍**：这是 10.6“价差三工程规则”的量化压缩。机制很简单：价差的最大盈利 = 价差距离 − 净成本，最大亏损 = 净成本；想让盈亏比站得住，净成本必须远小于价差距离。付一半以内（最好三分之一）不是拍脑袋，是把“最大盈利 / 最大亏损”压到 ≥1（甚至 2:1）的数学要求；
- **ATM 行权价 = 最宽的价格区间**：ATM 是 Gamma 最大的地方（10.4），Delta 对价格最敏感，所以标的稍微一动期权就跟着动；OTM 必须等标的走过行权价才开始有 Delta，等于把“反应区间”让出去一截。日内用期权尤其重要——时间短，等不起 OTM 那一段“白走”。买入略 OTM 只是为了凑 3:1 价差比时偶尔为之，不是因为它更容易赚；
- **波动性缩小，利润缩小**：期权快速达到目标后横盘，IV 会随动能消退而回落，同时 Theta 还在继续扣——即使标的不跌，期权价格也会缩水。所以退出要以**标的**的价格为指导，不要等期权本身的价格：你的方向观点来自价格行为，标的价是干净、可归因的风险锚；期权价里混着 IV 和时间两个你无法直接控制的变量。执行：标的触及止损时亏损出场，头寸达到风险两倍时部分获利，期权价值翻倍时部分或全部了结——把“什么时候该走”交给标的规则，而不是交给期权价格的随机波动。

这四条不是新策略，而是把 10.6 里已有策略的执行细节量化——它们共同回答“什么时候该动、什么时候该等、什么时候该走”。使用顺序可以串成一条检查：**先用第 3 条决定选 ATM 还是 OTM，用第 2 条检查价差盈亏比，用第 1 条定对冲期限，用第 4 条管理退出**。
