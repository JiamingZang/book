...
**进阶参考**：若想系统补期权定价与波动率理论，Natenberg《期权波动率与定价》（Option Volatility & Pricing）是经典入门+进阶参考；本书只取其中与价格行为交易者最相关的一小部分（10.3 定价、10.5 波动率、10.8 陷阱）。想继续深入，可优先看该书第 4 章波动率、第 8 章波动率价差、第 14 章再论波动率、第 16 章波动率关系与倾斜。

**期权的量化执行规则（Ali）**：Brooks 给的是“理解层”——先搞懂 IV、只用简单工具；Ali 给的是“执行层”——在简单工具里如何量化管理。两者不矛盾，理解层决定你敢不敢用，执行层决定你能不能用好：
