...
![图 9-1R 真实数据：同一批真实交易记录套上考核规则——0.5% 风险 82 笔全程没碰 -10% 总回撤线，1% 风险第 79 笔打穿 → 账户作废；先回测验证再买考核（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch9_eval.png)
*图 9-1R 真实数据：同一批真实交易记录，0.5% vs 1% 单笔风险在考核规则下的结局（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）*

**这张图要读出的三件事**（这比图 9-3 的合成示意更扎心，因为是真实数据）：

1. **同样的交易、不同的仓位 → 完全不同的结局**。0.5%：82 笔全程在 -10% 线之上，最大回撤 -5.6%，安全区；1%：第 79 笔打穿总回撤线，前 78 笔全部白费——**考核只认结果，碰线即出局，不看过程**。前 78 笔你做得再"对"，第 79 笔一样清零。
