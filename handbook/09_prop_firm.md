...
*图 9-2 轻仓 vs 重仓：0.5% 风险 80 笔稳步达标，2% 风险 20 笔左右撞线出局*
...
**这张图要读出的三件事**（这比图 9-2 的合成示意更扎心，因为是真实数据）：
...
**和 9.3 的数学对上了**：图 9-2 说的是"重仓把 1.5 个月压缩成 1-2 周，代价是淘汰概率拉到必然"；这张真实图就是那个"必然"的实证——1% 都嫌重（连 0.5% 的目标都过不了，何况翻倍）。"慢就是快"不是口号，是数学：**仓位不是"想赚多少"的旋钮，是"能不能活到概率兑现"的开关**。
...
*图 9-3 考核为什么失败：左=五类失败原因按频率排序（示意值），重仓冲刺最高、忽视一致性规则最低，全是纪律问题；右=方法 × 仓位 × 执行三角——三者缺一不可，考核通过是三者都满足的结果*
...
*图 9-4 一次完整考核的进度阶梯：8 周 72 笔稳稳 +10%（红线一次不碰）；Phase 2 与 Funded 摘要（编者按 9.8 数字绘制）*
