...
> **阅读提示**：本章不是平台测评，而是回答一个问题：**把 E 和 R 放进考核的硬约束里，怎么推才能活到通过并守住账户？** 先看 9.1 规则清单（盈利目标/回撤/期限），再读资金曲线管理、失败归因与考核进度阶梯；考核不是新系统，是同一套方法在不同约束下的应用。
...
**记住约束的本质（呼应第 6 章）**：日回撤 5% 意味着**全亏时**你一天最多错 10 次（单笔 0.5% 风险时）；若中间有盈利单，亏损笔数可以超过 10，但当日净亏损仍不能超过 5%。考核考的不是“你能赚多快”，是“你能不能稳定地不碰线”。

**考核设计的商业逻辑（看懂它，你就不焦虑了）**：平台卖的不是“让你赚钱”，是“考核费 + 对赌亏损”。大多数参与者亏掉考核费离场——平台的商业模式建立在“多数人过不了”上。所以考核的设计天然倾向：目标定得比散户的耐心长（1.5-2 个月），回撤定得比散户的纪律紧（碰线出局）。你的优势恰恰是多数人的反面：耐心 + 纪律。第 9.3 的数学会证明：**在系统期望值为正的前提下**（先回测验证，8.2/8.7），稳定执行的人通过是大概率事件（不是数学必然，9.3 的分布尾部仍存在不达标的可能）——注意这个前提，9.3 后半段的真实数据会展示没有它时的结局。
...
**选择不是比广告，是把你的系统约束投影到平台规则上**：

1. **先定品种再选平台**——因为方法依赖的数据不同：量价方法（Wyckoff/订单流）需要真实集中成交量，期货合约量是撮合出来的、可信；价格行为/SMC 只读结构，外汇平台即可。做量价方法选期货平台，做价格行为/SMC 选外汇平台。这一步不是偏好，是“你的信号需要什么数据”直接决定的。
2. **读全规则，对照你的交易时段**：周末持仓、新闻交易、一致性规则（consistency）、最低交易天数——这些规则不是平台在为难你，而是**把你的系统约束和平台的约束做交集**：你做美盘就选允许美盘活跃的平台，你做波段就避开周末必须平仓的平台。违规直接封号，前面努力清零。
3. **警惕过低的考核费**——如果考核费低到无法覆盖平台成本，利润一定从别处来：过高的点差、极难的一致性规则、出金拖延。这是成本结构决定的，不是“平台良心”。

**平台对比的实操维度**（不止看考核费）：① 规则透明度（规则文档是否完整、是否频繁改规则）——对应“你的规则能不能在它的规则里稳定执行”；② 出金口碑（搜索“平台名 + 出金/封号”看真实反馈）——对应“赚到钱后 E 能不能落袋”；③ 点差/佣金水平（第 6.6 讲过，点差是隐性成本，考核盘普遍点差偏大）——对应“你的净期望值被吃掉多少”；④ 客服响应（考核中遇到技术问题，客服态度决定你的体验）。考核费只是最表面的成本，别被“$49 挑战 $100k”的广告带着走。
...
每笔期望收益率 = 0.2 × 2% = 0.4%
8% → 理论只需 20 笔 ≈ 1-2 周
但：**若这 3 笔发生在同一交易日**，连亏 3 笔累计 ≈ −5.9%（0.98³），直接打穿 5% 日回撤线
同一交易日内连续 3 笔全亏的概率 = 0.6³ ≈ 21.6%（任取同一交易日连续 3 笔，约五分之一全是亏；跨日连亏打的是总回撤线，不是日回撤线）
长期重仓，必然撞上 → 淘汰
```
...
**上面是合成示意，下面用真实数据把同一个道理跑一遍**。还是图 8-1R 那 82 笔真实逐笔记录（EMA20/50 在 BTC 1H，期望为负的策略），套上考核规则：目标 +10%、日回撤 ≤5%、总回撤 ≤10%（本图按 10% 目标；后面图 9-2R/9-3R 改用 8% 目标——Phase 1 目标区间 8-10% 的下限，**门槛更低、更保守**，结论不变）。唯一变量是单笔风险——0.5% 还是 1%：
...
![图 9-3R 真实数据：考核通过率 Monte Carlo——同一批 82 笔真实交易 × 7 档单笔风险 × 重排 20000 次：低风险（0.25%/0.5%）平庸 100%（考核费无限续费）、1.0% 出局 38.7%/平庸 61.3%（两头不讨好）、1.5% 出局 99.1%（重仓送分）、3.0% 出局 85.9%/达标 14.1%；真实顺序那次 3.0% 第 2 笔就冲线达标（+2.85R 大单 × 3% = +8.55%）——但 MC 说 3.0% 档仍有 85.9% 概率出局，单次达标是运气不是方法（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch9_passrate.png)
...
2. **概率的全貌是“风险越高出局越快”**。低风险 0.25%/0.5%：平庸 100%，0% 达标、0% 出局（就是图 9-2R 的平庸重考）；1.0%：出局 38.7% 开始出现，但平庸仍 61.3%、达标 0%——两头不讨好；1.5% 起：出局率跳到 99.1%（平庸归零），2.0% 出局 95.4%，3.0% 出局 85.9% 但达标率升到 14.1%。**调高风险不是在“搏达标”，是在“送出局”**——最高达标率只有 14.1%，而出局率始终压在 86% 以上。
...
- **一致性规则**：很多平台要求不能靠单笔暴利（防重仓赌一把），保持稳定的单笔风险——具体比例与平台动机见 9.7。
...
- **一致性规则（consistency）**：部分平台要求单笔盈利不超过总盈利的某个比例（如 30-50%），防止“一把重仓赌对”。平台防的是“重仓一次碰线达标”的风格漂移，所以不能靠一笔大单达标，要分散盈利。**算例**：若总盈利 $1,000、平台上限 40%，则单笔最大盈利不能超过 $400——如果你某笔赚了 $450，即使总盈利达标，也会因一致性违规而不给通过。
- **周末持仓**：很多平台禁止持仓过周末——因为周末无法交易，周一跳空可能直接穿掉回撤线，平台要防这种“无法控制的风险”。周五收盘前平仓。
- **新闻交易**：部分平台禁止重大数据前后开仓——数据瞬间滑点大、价格行为失真，平台既防你被扫，也防你用信息差钻空子。
- **最低交易天数**：有的要求最少交易天数——防止“赌一把快速达标”和“只靠运气过线”，逼你展示一致性。
- **风格检测**：检测马丁、对冲、高频刷单等——这些策略的本质是把单笔风险转嫁给平台（马丁/网格/对冲锁仓），所以平台判定违规。
...
- **注意这里与 9.9 图 9-5R 的 $50,000 不是同一口径**：10,000-25,000 是“能承受学习期总亏损”的学费底线；$50,000 是“严格按 0.5% 单笔风险 + ES 最小止损 5 点”反推出来的考核期风控要求。若你坚持考核期 0.5% 风控，ES 需要 5 万；10-25K 是更宽的学费预算。
...
- 如果只能承受 $5,000 的亏损，别碰 ES：选低价高流动性的股票/ETF（$20 以下、日内波动 20 美分级），或者**SPY 周期权**——平值/略虚值的 SPY 近月期权通常 $1 以下（常低于 $0.50，视行权价与到期日而定），管理得当亏损常控制在 $0.20（即 $20）以内。这是把“学费”降到最低的合法途径：真实市场、真实金钱、真实情绪，但每次试错的成本接近零。
...
**季节性（日历观察，仅供参考）**：账户规模之外，还有一个只影响节奏、不影响入场的问题是季节性。Brooks 提到过几个有统计依据的日历规律，表格化如下：
