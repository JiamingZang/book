...
**答案**：① 假突破换了个名字（5.3/3.2）——扫止损的机制完全相同，SMC 是价格行为的换皮（前言立场 4）；② 止损单聚集在前高前低附近，机构需要对手盘，扫掉止损才有反向燃料（5.2）——所以止损必须挂结构外、有依据的位置；③ 入场前截图 + 写下理由（5.9）——事后解释不算数，只有入场前的判断才算，这是对抗记忆重构的唯一证据。
