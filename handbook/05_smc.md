...
四步对照：① **spring**：02:20 一根 K 线探到 61825，扫掉 61800 下方空头止损（SSL）后收回——诱空成立，但此时只是“警告”，不能直接抄底；② **横盘蓄势**：12:00-17:00 白天在 62500-63000 之间横盘，低点抬高（62500 → 62632），底部结构在形成——这是 spring 之后的“筑底期”，进场要等突破而不是猜；③ **突破确认**：20:30 巨量 K 线（V=1254，全天最大量之一）收盘站上 62900 平台，21:00-22:00 回踩不破 63500 二次确认——这里才是顺势入场点；④ **尾盘高潮**：23:30 加速冲到 64966 全天高点，当天 +3.4% 走完。注意 spring 到突破确认隔了 18 个小时，**中间没有任何值得抢跑的信号——等待本身就是 SMC 的核心纪律**。
...
![图 5-2R 真实数据：扫流动性 → CHoCH → 下跌趋势日（BTC 5 分钟，2026-07-31，全天 -2.7%）](images/fig_real_btc_sweep.png)

*图 5-2R 真实数据：sweep → CHoCH → 下跌趋势日（数据源：Binance BTCUSDT 5m K 线）*

四步对照上面的规则：① **sweep**：06:20 一根 K 线插破早盘高点 60846 到 61334，扫掉上方追多止损（BSL）后立刻收回——sweep 成立（5.3 识别清单全中）；② **CHoCH**：08:15 跌破前低 59876，且是强位移 K 线（严格定义），多头结构被破坏——此时**只停止做多，不急着反手**；③ **等新结构**：09:40 反抽 60325 不破前高，形成 LH + LL 序列，下跌结构确认，反抽位就是顺势做空入场点（止损放 LH 上方）；④ **纪律**：08:40 日内低点 59588（比 CHoCH 又低了 300 点），随后反抽无力，当天 -2.7% 走完。注意加密 5 分钟图里 sweep 和 CHoCH 常常间隔只有十几根 K 线（不像日线可以等一天），**等收盘确认的纪律在快节奏里更重要，别在插破瞬间抢跑**。
...
图 5-6 把 5.2-5.6 的知识点收拢成一个可执行的序列：顶部先问“背景允不允许做”，中部每一步都在回答“现在到了哪一步”，底部提醒你这不是新体系。**五步的顺序不能跳**——先有结构才知道 sweep 有意义；先确认 sweep 才知道 FVG/OB 是入场区而不是下跌中继；先有入场区才有执行和管理。对照表与图 5-7 就是同一件事的两种呈现：表格给你“每个 SMC 术语翻译成什么”，图给你“翻译后的系统长什么样”。
...
![图 5-8 Wyckoff 吸筹结构：PS→SC→AR→ST→Spring→SOS→LPS](images/fig_p41_x187.png)
...
![图 5-3R 真实数据：Volume Profile——HVN 堆积、LVN 快速区、POC 支撑（BTC 5 分钟，2026-08-11 ~ 08-13）](images/fig_real_volprof.png)

*图 5-3R 真实数据：Volume Profile（数据源：Binance BTCUSDT 5m K 线）*

左侧是 K 线，右侧是按价格横向累加的成交量分布（量往哪个价位堆，哪里就有磁力）。看三个真实现象：① **POC 支撑**：红线是最大成交量节点（POC ≈ 63400 附近）——8/13 价格回落测试 POC 后企稳，量分布就是最真实的支撑位；② **HVN 堆积**：蓝色底纹是量堆积区（HVN），价格在这里反复停留，8/11 整天都在该区间内震荡——VA 内按区间思维（系统二）；③ **LVN 快速区**：绿色底纹是量很少的快速通过区，8/12 18:00 放量（V=151）突破 64100 后价格一口气穿过 LVN 加速上行——**在 LVN 里挂单等回踩是接不住的，价格不会停留**。对照 5.12 的三句实战用法逐条验证：POC 与支撑重叠 → confluence 加分；VA 内区间、VA 外趋势；LVN 不挂单、直奔 HVN 再评估。
