...
![图 7-1R 真实数据：连亏不是故障，是分布——真实 82 笔回测的连亏段 vs 理论预期（上=连亏段长度分布，橙=真实 82 笔出现段数 vs 青=理论期望（40% 胜率×82 笔），真实“≥3 连亏 8 段”≈理论期望 7.0 段精确命中，最长 6 连亏（第 77-82 笔）标注“80.5% 概率会出现的正常连亏”；下=理论最长连亏分布直方图，红虚线=真实最长 6 连亏落在分布主体区，P(最长≥6)=80.5%、P(最长≥5)=94.9%）（数据源：同图 8-1R 的 82 笔真实记录）](images/fig_real_ch7_streaks.png)

*图 7-1R 真实数据：连亏不是故障，是分布——理论期望（青）与真实出现段数（橙）逐段对齐；最长 6 连亏（第 77-82 笔）位于 40% 胜率 82 笔样本的分布主体（P≥6=80.5%）*
...
![图 7-2R 真实数据：一页真实交易日志的统计视图——82 笔逐笔 R（每根柱子=日志一行，绿=赢/红=亏）与累计 R；连亏段高亮：最长 6 连亏是 40% 胜率下的正常分布，却也是心理最危险时刻（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch7_journal.png)
*图 7-2R 真实数据：一页真实交易日志的统计视图——逐笔 R 与累计 R，连亏段高亮（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）*
