...
![图 7-3R 真实数据：一页真实交易日志的统计视图——82 笔逐笔 R（每根柱子=日志一行，绿=赢/红=亏）与累计 R；连亏段高亮：最长 6 连亏是 40% 胜率下的正常分布，却也是心理最危险时刻（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch7_journal.png)
*图 7-3R 真实数据：一页真实交易日志的统计视图——逐笔 R 与累计 R，连亏段高亮（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）*
