...
![图 7-4 一个交易日的三时刻流程：盘前做计划（冷静）→ 盘中只执行（情绪被规则框死）→ 盘后做复盘（记录）；连接线是"边界物理化"——计划写在纸上、盘中不刷消息、复盘用固定模板](images/fig_p7_flow.png)
...
四种核心思维：频谱（状态是连续频谱非二元）、概率（趋势中多数反转失败、区间中多数突破失败）、惯性（默认延续，反转须趋势线突破+极点测试失败）、嵌套（大周期定方向、小周期定入场）。概率锚点：区间突破约 80% 失败、反转尝试约 80% 变旗形、完整 MTR 仅 35-40%（只诊断不下单）、尖峰后 60/30/10——末端三推/楔形反转时追顺势是最隐蔽的陷阱，应 wait。
