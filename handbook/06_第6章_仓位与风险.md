...
**例一（外汇 EURUSD）**：$100,000 账户，风险 0.5% = $500。入场 100.00，止损 99.50（50 点）。每点 $1/手 → 每手止损亏损 = 50 × $1 = $50 → 手数 = $500 ÷ $50 = 10 手。
...
![图 6-1 止损距离 × 仓位 = 恒定风险：止损翻倍，仓位减半](images/fig_p6_stop_size.png)

*图 6-1 止损距离 × 仓位 = 恒定风险：止损翻倍，仓位减半*
...
![图 6-2 连亏路径示意：40% 胜率系统里的正常样子（0.6^5 ≈ 7.8%，100 笔内至少一段 5 连亏 ≈ 99.9%）；连亏来了——错误的反应是怀疑系统/改规则/改仓位，正确的预期是"又来了"，按计划继续](images/fig_p6_losing_streak.png)

*图 6-2 连亏是必然：问题不是"怎么避免连亏"，而是"连亏时你的仓位能不能扛过去"——0.5% 风险保证你不会出局，概率迟早兑现*
...
![图 6-3 盈亏平衡胜率曲线：RR 越高，所需最低胜率越低](images/fig_p6_be_curve.png)

*图 6-3 盈亏平衡胜率曲线：RR 越高，所需最低胜率越低*
...
![图 6-1R 真实数据：ATR 波动率通道（BTC/USDT 5 分钟）：价格 ±1.5×ATR 形成动态通道，低波动期通道收窄、高波动期通道张开；顶部紫色窗格为 ATR(14) 数值](images/fig_real_atr.png)

*图 6-1R 真实数据：ATR 波动率通道（数据源：Binance BTCUSDT 5m，2026-07-01 15:00 ~ 23:15，100 根）：通道宽度 = 波动率本身*
...
![图 6-4 凯利公式：左=f* 随胜率变化曲线族（盈亏比 1/2/3），40% 胜率 + 盈亏比 2 对应 f*=10%；右=对数增长率曲线，满凯利是尖峰，半凯利（5%）损失极小但波动大幅缩小](images/fig_p6_kelly.png)

*图 6-4 凯利公式：左=f* 随胜率变化曲线族（盈亏比 1/2/3），40% 胜率 + 盈亏比 2 对应 f*=10%；右=对数增长率曲线，满凯利是尖峰，半凯利（5%）损失极小但波动大幅缩小*
...
![图 6-5 破产概率：左=100 笔内至少出现一段 k 连亏的概率（40% 胜率）——5 连亏约 100%、10 连亏约 42%、20 连亏趋近于零；右=犯错预算对比，2% 风险只有 5 次，0.5% 风险有 20 次](images/fig_p6_ruin.png)

*图 6-5 破产概率：左=100 笔内至少出现一段 k 连亏的概率（40% 胜率）——5 连亏约 100%、10 连亏约 42%、20 连亏趋近于零；右=犯错预算对比，2% 风险只有 5 次，0.5% 风险有 20 次*
...
![图 6-6 复利 vs 单利：月收益 4%（年化约 60%）持续 36 个月——复利滚到约 4.1 倍，单利只有 2.4 倍，差距全在"利滚利"](images/fig_p6_compound.png)

*图 6-6 复利 vs 单利：月收益 4%（年化约 60%）持续 36 个月——复利滚到约 4.1 倍，单利只有 2.4 倍，差距全在"利滚利"*
...
![图 6-7 加仓三边界：① 新手不做（要求精确判断"前提仍有效"）；② 只加 1-2 次且有计划（计划外加仓叫摊平）；③ 浮亏不加、盈利才加（摊平 = 把概率游戏变赌命）；对比：盈利加仓止损上移 vs 浮亏摊平越套越深](images/fig_p6_add_position.png)

*图 6-7 加仓是把双刃剑：用得好让利润奔跑（海龟 4.9），用得差是爆仓加速器——考核期完全不加仓，实盘稳定后再写进规则书*
...
![图 6-8 "我不在乎"的头寸规模：怕痛（仓位太大）→ 提前止盈/止损犹豫/报复加仓；仓位小到"亏了也不影响心情"→ 情绪燃料消失 → 执行一致（本身就是最大的优势来源）；常规仓位减 50-75% 做训练仓](images/fig_p6_size_psych.png)

*图 6-8 "我不在乎"的头寸规模：焦虑是信号不是性格缺陷——先找原因（仓位太大？逆势单？）；执行率稳定后再逐步放大，放大的是权益不是胆量*
...
![图 6-9 洛氏 3 手合约：第 1 手抵费用、第 2 手赚成本、第 3 手保本后奔跑](images/fig_p6_three_contracts.png)

*图 6-9 洛氏 3 手合约：第 1 手抵费用、第 2 手赚成本、第 3 手保本后奔跑*
...
![图 6-10 风险预算制：总预算 $750 → 按策略权重切分（40/30/30）；同向高相关持仓合并计算；期权对冲是组合层第 4 条安全带（编者示意）](images/fig_p6_risk_budget.png)

*图 6-10 风险预算制：总预算 $750 → 按策略权重切分（40/30/30）；同向高相关持仓合并计算；期权对冲是组合层第 4 条安全带（编者示意）*
...
宁可低胜率、高盈亏比：RR=2 只需 33% 胜率。

R 倍数 + 弹珠袋：系统是一袋弹珠，你控制每次押多少。

凯利只用半凯利以下；成本要扣进期望值。

交易者等式：奖励×胜率 > 风险×败率——风险和回报由你设定，概率交给市场；90% 的行情概率在 40-60%，2 倍回报让它们数学上有利。
