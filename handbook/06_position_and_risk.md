...
- **最佳系统约 5**；
- **Z < 2**：优势不显著，成绩可能只是运气（Z 为负则显著亏钱，如上面的 −2.0）。
...
代入看：E = +0.2R、风险 0.5%、回撤线 10% → (0.8/1.2)^20 ≈ 0.03%——几乎不可能；而 E 为负时底数大于 1，RoR 随笔数增加直奔 100%——**负期望系统不是“破产概率高”，是时间问题**（图 6-5R 的 82 笔实证）。公式是近似（固定分数下注的权益是乘性变化，比单位下注更抗连亏），但方向性结论完全可靠：E 的符号决定生死，风险百分比决定速度。
