...
![图 6-1R 真实数据：回本不对称——同一批 82 笔真实交易 × 7 档风险：0.5% 风险最大回撤 −5.6%，回本只需 +6.0%；1% → −11.0% → +12.4%；2% → −21.0% → +26.6%；3% → −30.0% → +42.8%；5% → −45.4% → +83.2%；10% → −71.8% → +255%；20% → −93.7% → +1487%（翻 15 倍都回不来）；理论曲线显示回本难度随回撤深度指数爆炸；垂直虚线为考核总回撤线 −10%——平台把回本成本锁在 +11.1% 以内，保证你“还能爬回来”；仓位是“未来回本难度”的开关，不是“赚多少”的旋钮（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch6_recover.png)
...
上面是纸上的概率，真实数据长什么样？用一套真实回测看（EMA20/50 趋势跟踪，BTC 1H，2026-07-02 ~ 08-10，82 笔）：胜率 40%，**最大连亏 6 笔**——和纸上算的“100 笔里几乎必然出现 5 连亏”完全对上。而且这套系统**验证不合格**（总 R −7.83，见 8.2 的图 8-1R；R = 单笔止损距离的风险单位，定义见 6.4）。**顺手把这 82 笔的账对平**：33 笔盈利平均 +0.41R、49 笔亏损平均 −0.44R（不是每笔都亏满 −1R，不少小亏单在前提失效时提前离场），所以胜率 40% × 0.41 − 60% × 0.44 ≈ −0.10R/笔，× 82 ≈ −7.83R——三个数字互相印证，盈亏比（平均盈 ÷ 平均亏的绝对值）= 0.94。但注意下面这张图：同样是这 82 笔交易，风险 0.5% 时最大回撤只有 −5.6%，1% 时 −11%，账户都活着；风险放大到 20%，回撤 −93.7%，几乎爆仓。

![图 6-2R 真实数据：同样的 82 笔交易，仓位决定你是亏 11% 还是爆仓——回撤 ≈ 随风险线性上升（−5.6% → −11% → −21% → −30% → −45% → −72% → −94%），期末净值 ≈ 随风险指数衰减（96% → 92% → 85% → 78% → 65% → 39% → 12%）；1% 风险下即使系统验证不合格账户也活着，20% 风险下几乎出局；虚线为典型 prop 考核总回撤上限 −10%](images/fig_real_ch6_risk.png)
...
**直觉**：把实际盈利笔数与“纯随机假设下应有的盈利笔数”之差，除以随机波动的标准差。本书采用可直接对账的算法——检验“实际胜率是否显著高于盈亏平衡胜率”（p₀ = 1/(1+RR)，6.4 的平衡表）：
...
**止损后价格回来了——单笔 -1R 的懊悔，就是概率规则最难熬的时刻**：上一节图 6-2R 用的是 82 笔真实回测（EMA20/50，BTC 1H，2026-07-02 ~ 08-10，每笔风险 0.5%）。其中最大的亏损单是这一笔：
...
- **（②）但“扛住”的代价不是多赚 1.17%，是尾部**：82 笔里 49 笔亏损全部被 -1R 锁死（图 6-2R 那条曲线的来源）；而不止损的版本里，这笔单在次日 07-20 16:35 的恐慌插针最低 63100——若扛单浮亏约 **-5.8R**（从 -1R 到 -5.8R，风险放大近 6 倍；见第 2 章图 2-4R 同一根插针）。概率规则没法保证每一笔都对，它保证的是**任何一笔都不会毁掉你**。
...
**动能转换时快速减回正常仓位（Brooks，35A）**：加仓的前提是“突破还是强势突破”——**一旦突破开始转换成其他结构（多头通道或交易区间），必须非常快速地把仓位减少到正常规模**。加仓者最常犯的错不是加错，而是加了之后不肯减：前提一变，先减仓再评估，别等回调把浮盈吃光（呼应 6.10 浮亏不加、7.4 连亏应对、4.17 信号失效主动退）。
