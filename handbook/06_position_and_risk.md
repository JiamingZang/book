...
图 6-1 算的是“能亏几次”，这一张算“亏了之后要爬多高的坡才能回来”。还是那 82 笔真实交易（EMA20/50 在 BTC 1H，与图 6-2R 同一序列、同一 7 档风险），先跑出每档的真实最大回撤，再换算成“回本所需涨幅”——数学本质就是本节开头那张表：**亏损是乘性的，亏得越深，回本越贵**：

![图 6-1R 真实数据：回本不对称——同一批 82 笔真实交易 × 7 档风险：0.5% 风险最大回撤 −5.6%，回本只需 +6.0%；1% → −11.0% → +12.4%；2% → −21.0% → +26.6%；5% → −45.4% → +83.2%；10% → −71.8% → +255%；20% → −93.7% → +1487%（翻 15 倍都回不来）；理论曲线显示回本难度随回撤深度指数爆炸；垂直虚线为考核总回撤线 −10%——平台把回本成本锁在 +11.1% 以内，保证你“还能爬回来”；仓位是“未来回本难度”的开关，不是“赚多少”的旋钮（数据源：Binance BTCUSDT 5m K 线重采样 1H 逐笔回测记录）](images/fig_real_ch6_recover.png)
...
2. **这解释了平台为什么把总回撤线设在 −10%**。−10% 的回本成本是 +11.1%，考核期内还能爬回来；如果线设在 −30%，回本要 +42.8%，1.5 个月几乎不可能——**平台设 −10% 不是心软，是让“重考”还有意义**。你重仓把回撤打穿出局，等于亲手把“回本的坡”变成“爬不上的墙”。
...
**高概率交易的正确用法——目标刻度（Brooks）**：高概率不是不能做，而是目标要按概率调档：**高概率交易选 1 倍风险获利（“合理位置”，合理≠理想），低概率交易选 2 倍风险获利——赢的概率和赚的倍数互相抵消，期望值才站得住**（这就是上文“三个变量不可能同时占优”的实操版）。含义：区间顶/底刮头皮（70-90% 胜率）目标就定 1R，别贪 2R 以上的幻想；波段反转（40% 胜率、TBTL）目标定 2R 才划算——**胜率越低，目标必须越高，否则你在用高概率的盈亏比做低概率的交易**。
...
- **（②）但“扛住”的代价不是多赚 1.17%，是尾部**：82 笔里 49 笔亏损全部被 -1R 锁死（图 6-2R 那条曲线的来源）；而不止损的版本里，这笔单在次日 07-20 16:35 的恐慌插针最低 63100——若扛单浮亏约 **-5.8R**（从 -1R 到 -5.8R，风险放大近 6 倍；见 2-4R 同一根插针）。概率规则没法保证每一笔都对，它保证的是**任何一笔都不会毁掉你**。
