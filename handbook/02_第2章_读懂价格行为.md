...
对照图 2-4 的概念逐项看：① **spring 启动**：09:10 一根 K 线放出全天最大量（V=1551）并砸出 494 点下影（57800），扫掉 58000 下方的空头止损后收回——这不是普通回调，是趋势启动前的洗盘（第 5 章的 sweep 同款逻辑）；② **HH/HL 结构**：09:35 放量突破 58500 后，四个更高高点（58933→59032→59315→59457）与四个更高低点（58225→58775→58906→59154）清晰排列——趋势 = HH + HL，不是单看 K 线方向；③ **浅回调**：13:05-13:35 回调到 59126-59292，低点高于前一个 HL——顺势回调（H2）就是入场点，止损放回调低点下方。整张图就是"定义→识别→入场"的完整演示。
...
![图 2-2R 真实数据：两段式移动——双底 → 第一腿 → 回调 → 第二腿等幅（BTC 5 分钟，2026-07-14 ~ 07-15）](images/fig_real_eth_2leg.png)

*图 2-2R 真实数据：两段式移动（数据源：Binance BTCUSDT 5m K 线）*

图里四步可复刻：① 19:20 和 20:05 双底 62762/62780——两腿前的底部结构，第二根高一点（更高低点）就是 2.3 的结构确认；② 21:20 大实体第一腿 64182（+2.2%），突破双底颈线，顺势者入场；③ 22:00 回调 63581（回撤约 40%）——**不是反转，是第二腿前的蓄势**，顺势入场的止损放回调低点下方，目标看等幅第二腿；④ 23:30 第二腿 64966（+2.2%，涨幅≈第一腿，量度移动的雏形），两腿走完后高位回落（01:10 起 -1%+）。记住：**两段式移动的钱在第二腿的回调里，不在第一腿的突破里**。
...
**卖出高潮（Selling Climax）与买进高潮的关键区别**：识别特征完全镜像（长下尾 > 实体 50% / 小实体 < 均值 30% / 反向棒），但后续行为不同——**卖出高潮后更倾向 V 形反转**：恐慌抛售后空头“弹尽粮绝”，反弹往往与下跌一样快、至少回撤 38.2-50%；买进高潮后通常需要更长时间的盘整。**反转确认关键**：反弹棒出现后的第一根反向棒是否创新低——创新低 = 尖峰延续；不创新低 = 反转概率上升（仅诊断，不逆势）。

**高潮 K 线的量化预期（突破）**：高潮 K 线（climax bar）有 70% 的概率在 1-2 根 K 线内出现回调，超过 60% 的概率至少出现一个小幅第二波段（顺原方向）；当高潮信号棒出现在**末段行情**（趋势方向的最后 1-2 波）中时，通常导致更持久的回调甚至直接反转——与 2.11 的“第三次高潮是趋势末日”互证。所以高潮后的标准动作：**等回调（70% 会来），不追高潮**。
...
**尖峰后的三条路径（概率排序）**：① 转通道延续（约 60%——回调 < 50%、通道倾斜向上，通道下轨做多）；② 进入区间（约 30%——顺 direction 一侧边界操作）；③ 反转（约 10%——仅诊断，不逆势）。

**连续高潮与反弹容忍度**：同向两次高潮之间通常间隔 ≥ 10 棒，中间反弹常为两条腿；第二波力度与第一波相当或更大才算连续高潮（明显更弱 = 只是反弹）；**第三次高潮常呈抛物线，是趋势最后阶段**——之后通常大幅反弹或反转，禁止追单。**尖峰后反弹容忍度三档**：强尖峰后反弹不应超过尖峰高度 50%（继续顺势）；超过 61.8% = 尖峰失败、按区间场景处理；低于 23.6% = 极强趋势（小幅反弹趋势日）。

**衰竭性高潮（EGO）的识别与确认（Ali）**：一系列买入（或卖出）高潮的末期，会出现**最大、最强壮的 K 线**——看似新突破，实为最后疯狂，随后失败反转。三个细节：① **高潮后约 10 根 K 线内不宜反向交易**（见下文交易纪律），且至少 50% 概率会测试前一个高潮高点；② **真正的反转确认是高潮后的第一次回调（PB）**——高潮本身只是诊断；先出现带 PB 的最终 EG、之后才是反转的情况也很常见（EGO：EG 之后还有 EGO）；③ **3 次或以上的连续高潮（CC）且仅伴随 1-2 根回调线，通常形成衰竭趋势**——直到突破缺口被回补、该供应区被标记为耗尽缺口。TR 时期最强的趋势柱形成在每条腿末端附近——那是顶部/底部的高潮（EG 而非 MG），是对交易者的警示信号：当前走势即将反转，而非新的突破柱。
...
尖峰是趋势的引擎：2 根以上连续大型趋势棒、实体几乎无重叠、短尾线、收盘创新高——1 根只是候选；6 根以上进高潮预警，但高潮以衰竭信号为准（长上尾 > 实体 50% / 小实体 < 均值 30% / 反向棒）；尖峰后 60% 转通道、30% 进区间、10% 反转；只做回撤不追尖峰，高潮后至少等 10 棒。卖出高潮倾向 V 形反转（买进高潮需更久盘整）；反弹超 61.8% = 尖峰失败进区间、低于 23.6% = 极强趋势日；连续三次高潮是趋势末日。
